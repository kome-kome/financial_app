...
class TestParamsSchema:
    ...
    def test_macro_features_default(self):
        ...
        # ICE BofA 系列は既定から外れる（選択肢としては残る）
        assert "macro_hy_oas_zscore" not in result["macro_features"]
        assert "macro_ig_oas_zscore" not in result["macro_features"]
...
class TestExecuteIntegration:
    ...
    def _build_mock_db(self, ref: date = date(2025, 6, 1), n_weeks: int = 210):
        ...
        # ため（Issue #311）、順序ではなく args[0] の str 表現（"table.column" / クラス repr）で
        # 振り分ける。
        def _query_side_effect(*args):
            ...
            # str(InstrumentedAttribute)="Class.attr"、str(クラス)="<class 'database.Class'>"。
