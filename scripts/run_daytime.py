...
JOBS: dict[str, Job] = {
    ...
    "gate:ttm": Job(
        ...
        # **未実測**。M-2/M-6 の CV は1回 30〜70秒（`bakeoff_full.json`）で、2条件 × 2モデル ×
        # （生・共通月）の8回。キャッシュの取り直しとパネル構築（9/18 の列数モードで約10分）を
        # 足し、9/17 の `oof:split-bias`（M-2/M-6 の前後比較・8.6分）より重いと見て置いた。
        measured_min=30.0,
