...
初回実測（2026-07・PR#354）の無効化と再測（Issue #356）
--------------------------------------------------------
初回実測は「2/4/6/8 チェーンを通じ r_hat_max=1.0100 で完全平坦（＝プールでは下がらない・
1.01–1.02 はモデル幾何の床）」と結論したが、**この平坦は測定解像度のアーティファクトだった**。
当時の summarize_diagnostics は az.summary を既定引数で呼んでおり、arviz は r_hat を小数2桁へ
固定丸めする（ess は整数）。すなわち観測できた値は 1.00/1.01/1.02 の3段階しかなく、真値が
1.005→1.014 の範囲で単調に減っていたとしても、すべて 1.01 と表示される。

Issue #356 で summarize_diagnostics を round_to="none"（生値）へ修正済み。本スクリプトは
その診断関数をそのまま再利用するため、再実行すれば 4桁精度の傾向が得られる。
「プールで下がるか」「床は本物か」は、いずれもこの生値での再測をもって判断すること
（初回実測の数値は根拠として引用しない）。

生値での再実測結果（2026-08-02・500銘柄・K=4・draws=800・target_accept=0.95・numpyro）::

    tune=800  (run 30724216104): 1.0194(2ch) -> 1.0108(4ch) -> 1.0128(6ch) -> 1.0105(8ch)
    tune=2000 (run 30725731129): 1.0279(2ch) -> 1.0162(4ch) -> 1.0183(6ch) -> 1.0158(8ch)

- **プールは r_hat を下げる**（2→8ch で -0.0089）。初回実測の「完全平坦」は丸めの産物だった。
- ただし 8ch でも strict 1.01 未達（1.0105）。本番規模 3800銘柄では max 順序統計でさらに不利。
- **tune 増は逆効果**（全チェーン数で悪化・ESS も 214.8→157.2）。draws を据え置いたまま
  warmup だけ延ばすと、target_accept=0.95 の下でステップがより保守的になり探索効率が落ちる。

詳細は ADR-0002 の「改訂（2026-08-02・Issue #356）」節を参照。
