...
ローカル読取レプリカは pull / sync した時点で止まる。夜間バッチは Supabase 側に書くので、
**同期を忘れたミラーは古いスコアを最新の顔で見せ続ける**。これは #438（Yahoo の静かな
配信停止）と同型の「気づけない劣化」で、`/api/stats` の `freshness` も
`/api/system/info` のバッジも「どちらの DB か」は見せても「同期がどれだけ遅れているか」は
答えない。本スクリプトがその答えを持つ。
