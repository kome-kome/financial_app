...
「完走見込みのあるステップにだけ予算を与える」を原則に、`scripts/run_monthly_m1.py`
（毎月2日 01:00）へ切り出した（#584）。**#638・ADR-0054 で打ち切りは全損ではなくなったが、
この切り出しは覆さない**——畳んで残るのは探索空間の一部を見た結果であって、毎月それでよい
わけではない。
...
TUNE_MATRIX: tuple[tuple[str, str, tuple[str, ...]], ...] = (
    ...
    # 入らない。`scripts/run_monthly_m1.py`（毎月2日）へ切り出した（#584）。中途半端な予算は
    # #638 以後も「毎月ごく一部しか見ない探索」になるだけで、切り出しの根拠は変わらない。
    ("macro_dlm", "grid", ()),
...
BUDGET_MIN: dict[str, float] = {
    ...
    # 話**で、400分での結果ではない——その後 #584 で M-1 が出ていったぶんを 400 へ振ってある。
    # それでも 369分に対する余裕は 8.4% しかなく、下の macro_gbdt に「余裕2%では次回落ちる」
    # として約34%を積んだ基準を、**こちらにだけ適用していなかった**（#633）。同じ基準で
    # 500分＝約36%の余裕。パネルは毎晩伸びるので所要は据え置かず伸びる。
    # 当時は完走してからしか永続化せず、薄い予算は「遅れる」ではなく**その月ぶんが丸ごと
    # 消える**を意味した（7/10 の μ̂ が 59.5日 固着した実体がこれ）。#638・ADR-0054 で
    # 畳んで残すようにしたが、**予算を薄くしてよい理由にはならない**——畳んだ回は探索空間の
    # 一部しか見ていない。
    # マージンは `tests/test_run_monthly.py` が `run_daytime.JOBS[...].measured_min` と照合する。
    "tune:macro_dlm": 500,
    ...
    # （今回 0.2359）として persist をスキップしたため。**#590・ADR-0047 で解消**した
    # （0.5068 は 10 fold・0.2359 は伸びたパネルでの値で、比較が成立していなかった）。
    # 以後このステップは水準が下がっても exit 0 で persist する。所要とは別の論点。
    "tune:macro_gbdt": 240,
}
