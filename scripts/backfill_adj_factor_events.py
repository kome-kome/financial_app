...
対象を選ぶ。無指定なら検出器の既定（`DEFAULT_CONSISTENCY_CROSSCHECK`・2026-09-26 から ON）に従うので、
今は同じ対象になる。照合を既定で入れる前に、それで倍率待ちになる期末後分割の社と、翌年の株数で倍率を
決めた社の公式を先に取り込んで測るために足した（取り込み自体は係数表を変えない）。
