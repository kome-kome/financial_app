...
**2026-08-20（#503・ADR-0038）以降、ここで作る器は「レプリカ」ではなく正本である。**
用途は3つに増えた: ①新しいマシンで正本を立ち上げる ②バックアップの復元先を用意する
（`backup_restore.py --create-schema` が同じ `Base.metadata` を使う）③スキーマ移行の反映。
接続先ガード（下記1）の重みは**むしろ増した**——誤射先の Supabase は凍結した断面なので、
DDL を打てば復元の土台そのものが壊れる。
