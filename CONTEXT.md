...
[[比較ファミリー]]の第4メンバーだが、M-1〜M-3のような独立モデルではなく**基底モデルの[[アウトオブサンプル検証|無リークOOF]]予測 μ̂ を統合する二段目**。基底は `BASE_MODELS` 定数で決まり、M-1（線形）＋M-2（非線形）＋M-6（正則化線形・#397 で追加）の3つ。各モデルの per-(ym,銘柄) OOF を (ym, edinet_code) の intersection で整列し、二段ウォークフォワード（月 t の統合重みは t 未満の共通 OOF だけで NNLS 非負和1学習）で合成することで、単一モデル選択より安定した honest μ̂ を狙う。重みが非負・和1のため**効かない基底は自動的に重み ~0 へ落ちる**＝基底追加の下振れリスクが小さい。M-3 は週次専用（[[比較ファミリー]]の母集団が月次と異なる）、M-5 は順位スコアで水準を持たないため対象外。基底が増えると共通域は狭まりうるので、優劣判定は同一共通域に制限した各基底の OOF（`base_oof_backtest`）と比較する。現在の統合 μ̂ は `macro_ensemble_scores` へ永続化され、[[μ出所トグル]]（`mu_source`）の選択肢に `macro_ensemble` として加わる。
