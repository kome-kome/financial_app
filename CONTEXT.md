...
[[比較ファミリー]]の第4メンバーだが、M-1〜M-3のような独立モデルではなく**M-1（線形）とM-2（非線形）の[[アウトオブサンプル検証|無リークOOF]]予測 μ̂ を統合する二段目**。両モデルの per-(ym,銘柄) OOF を (ym, edinet_code) の intersection で整列し、二段ウォークフォワード（月 t の統合重みは t 未満の共通 OOF だけで NNLS 非負和1学習）で合成することで、単一モデル選択より安定した honest μ̂ を狙う。M-3 は週次専用（[[比較ファミリー]]の母集団が月次と異なる）のため初版は対象外。現在の統合 μ̂ は `macro_ensemble_scores` へ永続化され、[[μ出所トグル]]（`mu_source`）の選択肢に `macro_ensemble` として加わる。
