    ...
    def params_schema(self) -> dict:
        return {
            ...
            "use_momentum": {
                ...
                "label": "モメンタム特徴量を使用",
                "description": (
                    "ON にしても成績は良くなりません（実測済み・既定 OFF）。"
                    "12-1ヶ月モメンタムを特徴量に加える設定です。M-2/M-6 で ON/OFF を同一 fold・"
                    "同一銘柄集合で比較したところ、買い側の予測精度（rank-IC）も売り側の識別力も"
                    "改善せず（4検定すべて有意差なし・点推定はいずれも悪化方向）、"
                    "銘柄の入れ替わりが増えて売買コスト耐性だけが落ちました。詳細は ADR-0045。"
                ),
                "default": False,
            },
            "momentum_window": {
