...
def fit_view_metric_stats(records: list, weights: dict) -> dict:
    ...
    **`records` の `year` が混ざるのは正常**（Issue #520・2026-08-25 に実測して決着）。
    `latest_year_subq` は各社の最新年度を1行ずつ返すので、実測では 2025年度 1,249社 /
    2026年度 2,475社（33.5% : 66.5%）と**3社に1社が別年度**になる。これは提出の遅速ではなく
    **決算期の違い**で、year=2025 の中身は 2025-12 期(537社)・2025-09 期(189社)・2025-06 期
    (157社)、year=2026 はほぼ 2026-03 期(2,141社)。直近決算からの経過はどちらも 5〜8ヶ月で
    鮮度は同程度＝**「同一時点で入手可能な各社の最新財務」という正しい断面**である。

    したがって**年度別に標準化し直さない**。年度で切ると 12月期決算と3月期決算を別々の分布で
    評価することになり、「同じ市場で割安な銘柄を探す」という目的に反する。年度別標準化に
    変えると順位は動く（Spearman 0.908・上位100の重複 76%・上位100 の 2025年度組が 43→26社）
    が、**動いた先が正しいという根拠は無い**——同一企業で両年度を持つ 1,723社の `gap_ratio` は
    median diff +1.11・2026 が大きい会社 51.4% と**系統的なシフトが無い**ため、年度別の
    mean 差（+54.2 vs +42.7）は母集団の構成差であって混在が持ち込んだ歪みではない。

    なお「`gap_ratio` だけが年度正規化を通らないので危ない」という非対称も**成り立たない**。
    VIEW の `z_*` も年度で sd が桁違いに変わる（実測 `z_op_margin` は 2025年度 0.0159 /
    2026年度 0.0003 ＝ 53倍）。上の #469 の機構がそのまま出ているだけで、**この関数が断面で
    作り直す (mean, sd) はまさにその歪みを取り除いている**。
