...
#
# **#456 の再判定（2026-08-06・#411 バックフィル後パネル）**: 上2つの判定はいずれも週次株価
# キャッシュが #411（ADR-0025）の履歴バックフィル前で、パネルが 2021-01-08 起点へ切り詰められて
# いた（経緯は下の `DEFAULT_MACRO_FEATURES` 直上）。3,982社・67期・964,466 OOF ペアで測り直しても
...
# 無影響）。#454 は旧パネル（3,979社・49期・652,247 OOF）で rank-IC diff −0.0006・p=0.084 を出し、
# 「負方向かつ 2 検定中で最も α に近い」ため #456 で追跡することにした。
#
# **その旧パネル自体が #411 のバックフィル前だった。** M-3 の bakeoff が使う週次株価キャッシュ
# `scripts/.cache/weekly_prices_full_v1.pkl` は 2021-01-08 起点・967,004 行で、ADR-0025 の履歴
# バックフィル**前**の DB 状態（現在の DB は 2019-08-02 起点・1,282,435 行）。M-2/M-6 側が使う
# `weekly_prices_close.pkl` はバックフィル後だったため、**M-3 の昇格ゲートだけが古いパネルで
# 判定されていた**。`scripts/_cache.py` のキーはデータ世代を含まないので、mtime が新しくても中身は
# 旧世代でありうる（[[feedback_bakeoff_cache_generation_after_data_fix]]）。測る前に起点日を見る。
#
# バックフィル後パネル（3,982社・67期・964,466 OOF ペア＝期 +37%／サンプル +48%）で測り直した
# 結果、**負方向の懸念は消え既定維持で確定**——rank-IC +0.0122→+0.0122（diff −0.0001・p=0.695）
# ／売り側 spread +0.0011→+0.0012（diff +0.0000・p=0.206・符号は正へ転じた）。効果量が 1/6 に
# 縮み p が 0.084→0.695 へ動いた＝#454 の負方向は旧パネル固有のノイズだった。状態次元 21→22 の
...
def load_prices(db) -> tuple[dict, dict]:
    ...
    plugins/macro_snapshots.py の load_data と同じパターンを `shared_cache_get_or_compute`
    経由で再利用）。コンテキスト外では常にフル計算する。
    """
    from .macro_snapshots import shared_cache_get_or_compute
    return shared_cache_get_or_compute("load_prices", id(db), lambda: _load_prices_impl(db))
...
def load_macro_levels(db, series_codes: list[str], min_date: str | None) -> dict:
    ...
    shared_snapshot_cache() コンテキスト内では (id(db), series_codes, min_date) の
