    ...
    def params_schema(self) -> dict:
        return {
            ...
            "use_momentum": {
                ...
                "label": "モメンタム特徴量を使用",
                "description": (
                    "兄弟モデル（M-2/M-6）では ON にしても成績は良くなりませんでした"
                    "（実測済み・ADR-0045）。M-1 は母集団の作り方が違うため未実測ですが、"
                    "既定は OFF を据え置いています（#583）。12-1ヶ月モメンタムを特徴量に加える設定で、"
                    "マクロとは独立に切替可能。ON は各スナップショットに過去の株価履歴を要求します"
                    "（週次履歴は現在7年分あるので ON でも walk-forward CV は成立します）。"
                ),
                "default": False,
            },
            "momentum_window": {
