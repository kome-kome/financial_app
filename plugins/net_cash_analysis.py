...
class NetCashAnalysisPlugin(AnalysisPlugin):
    ...
    @staticmethod
    def _build_row(r, nc: float, ratio: float, ncav, ncav_ratio) -> dict:
        """1社分のレスポンス辞書を組み立てる。"""
        ...
        return {
            ...
            "industry":              r.industry,
            "year":                  r.year,
            "net_cash_oku":          round(nc / 100_000_000, 2),
            ...
            "market_cap_oku":        round(r.market_cap / 100, 2),
    ...
    def _apply_filters(self, records: list, params: dict, sanity_cap) -> tuple[list[dict], dict]:
        """nc_ratio / ncav_ratio / バリュートラップフィルタを適用し (rows, 除外統計) を返す。"""
        ...
        for r in records:
            ...
            if r.bs_investment_securities and r.bs_investment_securities > 0:
                n_inv_securities += 1
            rows.append(self._build_row(r, nc, ratio, ncav, ncav_ratio))

        return rows, {
            "n_inv_securities":  n_inv_securities,
            "n_excluded_sanity": n_excluded_sanity,
    ...
    async def execute(self, params: dict, db: Any) -> dict:
        ...
        sanity_cap = sanity_cap if (sanity_cap and sanity_cap > 0) else None
        top_n      = params["top_n"]
        sort       = params["sort"]

        records = self._build_query(db, params).all()
        rows, stats = self._apply_filters(records, params, sanity_cap)
        ...
        return {
            ...
            "n_inv_securities":   stats["n_inv_securities"],
            "n_excluded_sanity":  stats["n_excluded_sanity"],
            "n_excluded_trap":    stats["n_excluded_trap"],
            "thresholds": {
