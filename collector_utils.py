...
# 上の間隔を「株価履歴を持つ廃止社」にも掛ける閾値（日・Issue #556）。価格ゼロ社だけを
# 絞ったので、履歴を持つ廃止社は毎晩対象に残り、Yahoo は 404 を返し続けていた
# （2026-09-11 実測 263社＝毎晩の 4xx 約320件の大半）。**閾値は最終株価からの暦日**で、
# 生きている社は毎晩取るので 1〜3日にしかならない。#463 の誤 delisted（新規上場）も
# 当たり日に一度履歴を得れば翌晩から lag 1日に戻るので巻き込まない。該当 263社は全社が
# 60日超前で止まっており 8〜60日のどこに置いても同じ集合になるが、売買停止で一時的に
# 止まった社を巻き込みにくいよう余裕を持たせる。
DELISTED_STALE_DAYS = 30
