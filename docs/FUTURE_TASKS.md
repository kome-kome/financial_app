...
- **残課題**:
  - ~~**スピンダウン回避**: Free プラン 15 分アイドルで停止し `_daily_scheduler`（毎日3時）が走らない~~
    → **対応 (2026-05)**: `api.py:_startup_catchup()` でスピンアップ時に「最終自動収集から 22h
    以上経過していたら差分収集＋市場データ更新＋マクロ更新を非同期実行」する仕組みを追加。
    ユーザーアクセス（スピンアップ）のタイミングで自動補完されるため、Free プランでも実質
    1 日 1 回以上のデータ更新が保たれる。なお完全自動化（誰もアクセスしなくても更新）が必要なら
    外部 ping（cron-job.org 等を 14 分間隔）か有料プラン化を別途検討
