...
### O-1. DB バックアップ運用ポリシーの策定  【✅ 完了・2026-06-18】
- 自動バックアップ仕様（Free プラン: 1日1回・7日保持・PITR非対応）を文書化。
- 手動バックアップ（`pg_dump`）・Supabase ダッシュボードからの復元手順・プロジェクト停止からの復旧手順を `docs/DEPLOYMENT.md`「バックアップ運用ポリシー」節に追記。
