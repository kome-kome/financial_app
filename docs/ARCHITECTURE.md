...
graph LR
    ...
    subgraph ANALYSIS["📊 分析 /api/"]
        ...
        AN12["POST /api/plugins/{name}/tune-start\nハイパーパラメータ探索をバックグラウンド起動\n（M-1/M-2/M-3限定・常にRENDER_LIGHT_MODEで403・collection_jobs流用）"]
        AN13["POST /api/plugins/{name}/tune-stop\n探索ジョブへ停止リクエスト"]
        AN14["GET /api/plugins/{name}/tune-status\n探索ジョブのスナップショット（running/progress/total/recent_logs）"]
        AN15["GET /api/plugins/{name}/tune-stream\nSSE: 探索進捗配信（完了後 tuned バッジが自動更新）"]
...
| `routers/*.py` | バックエンド | `api.py` が `include_router` で束ねる REST ルーター4本（いずれも `APIRouter()`・フルパス保持）。`auth`（認証・Cookie/CSRF 発行）/ `collect`（収集管理・進捗SSE）/ `market`（統計・企業一覧・株価/履歴・マクロ・CSV）/ `analysis`（プラグイン実行・推薦・乖離・スクリーニング・バックテスト・DBビューア・ハイパーパラメータ探索ジョブ）。REST ルート定義の実体はここにある | database.py, collection_jobs.py, plugins/, data_quality.py |
| `collection_jobs.py` | バックエンド | 収集ジョブの実行時状態を集約する registry。job 名キーの `JobState`（running/progress/log/cancel）＋ start/cancel/snapshot/stream を提供。旧6本の並列 status dict を1箇所に畳む。SSE 配信ジェネレータ（`_sse_stream`）を内包。収集ジョブ専用ではなく job 名キーが衝突しなければ任意のバックグラウンドジョブに使え、ハイパーパラメータ探索（`tuning_{model}`・Issue #278）もこの registry を再利用する | fastapi |
...
| `hyperparameter_search.py` | GitHub Actions / ユーティリティ | M-1/M-2/M-3 ハイパーパラメータ自動探索（ADR-0007・Issue #264/#278/#291/#292）。CLI（`--model`＋`--persist`/`--persist-scores`）と GUI ジョブ（`routers/analysis.py` の `tune-start`）の両方が共有ロジック `run_search()` を呼ぶ（Issue #278 で CLI 専用の `_run()` から抽出）。`tuning_search_space()` を `plugins/tuning.py::search()` で評価し、`persist=True` で best params を `plugin_tuned_params` へ永続化、`persist_scores=True` 併用で最終 `execute_plugin` を1回実行し producer スコアを実反映する。**品質ゲート（Issue #291）**: `persist=True` 時、`plugin_tuned_params` に既存行があればその `objective_value` と今回の `best_score` を比較し、劣化していれば `upsert_tuned_params`（persist_scores 込み）をスキップして `result["skipped_reason"]` を設定・CLI は非ゼロ終了する（初回＝該当行なしはゲート対象外）。GitHub Actions 月次自動実行（`tune-hyperparameters.yml`・Issue #292）を人手レビュー無しで運用しても本番値が悪化しないようにするための仕組み。新規 pip 依存なし（`requirements-inference.txt` 分離は不要） | database.py, plugins/tuning.py |
