...
| `scripts/run_monthly.py` | ユーティリティ | **ローカル月次バッチの実体**（#504・親 #503）。`recommend_factor_premia.py` → `macro_beta_inference.py` → `hyperparameter_search.py` ×3（M-1 / M-3 / M-2）の順に回す。**並びは「依存順 ∧ 軽い順」**＝`macro_beta_loadings` は M-1 の入力なので推論が tune より先、かつ打ち切られても前方が揃うよう軽い順。引数は GHA の3本（tune / macro-beta / factor-premia）から**そのまま移設**（探索規模を変えると #291 の品質ゲートが別条件の値と比較される）。足跡は `monthly_last_run` / `monthly_last_success`（日次と別キー＝月次の停止が日次の成功で隠れない） | scripts/batch_common.py, recommend_factor_premia, macro_beta_inference, hyperparameter_search |
