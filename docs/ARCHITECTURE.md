...
| `plugins/sell_ranking.py` | バックエンド | 売り候補ランキング（割高度・業績悪化・nc_ratio 毀損・モメンタム）。`mu_source` で producer 切替（既定 M-6）・R3 足切りゲート・`depends_on=["sector_ols"]`・`ui_order=510`。詳細は [PLUGIN_REFERENCE.md](PLUGIN_REFERENCE.md#pluginssell_rankingpy) | plugins/utils.py, database.py, plugins.net_cash_analysis |
| `plugins/sector_ols.py` | バックエンド | 業種別OLS回帰分析（次元整合・winsorize+z-score 前処理）。予測値を `regression_results` へ保存・`heavy=True`・`ui_order=210`。詳細は [PLUGIN_REFERENCE.md](PLUGIN_REFERENCE.md#pluginssector_olspy) | plugins/utils.py |
...
| `plugins/utils.py` | バックエンド | `coerce_params()`・`ols()`・`normalize()`・`winsorize()`・`walk_forward_cv_monthly()`・`get_macro_features()` 等の共通ユーティリティ。ベクトル化後も旧実装とビット単位で同一値。詳細は [PLUGIN_REFERENCE.md](PLUGIN_REFERENCE.md#pluginsutilspy) | — |
| `tests/` | テスト | pytest 回帰テスト（1,646件 / 78ファイル・2026-08-09 時点）。ドキュメント同期のメタ検査（`test_docs_sync.py`＝MODELS.md §9 の章立てと M-1 副読本・#472）を含む。プラグイン＋utils＋`database.py`（upsert・RegressionResult merge・derived非永続）＋`collector.py`（XBRLパース・派生指標＋ネットワーク取得を httpx MockTransport でモック）＋`api.py`（純関数・`/health`・DB-backed 読取・heavy回帰のRenderブロック）をカバー。in-memory SQLite fixture（StaticPool）／FastAPI TestClient／httpx MockTransport で検証。`financial_metrics` は SQLite では `FinancialMetric` 列定義から生成したテーブルで代替し、派生値・予測値はテストが直接注入（`make_metric`）。計算式の同値性は Postgres で別途検証。共通 fixture は `tests/conftest.py`（`db`/`make_fin`/`make_metric` 等） | pytest, sqlalchemy, fastapi, httpx |
