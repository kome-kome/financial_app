...
| `plugins/base.py` | バックエンド | 分析プラグインの抽象基底クラス（`heavy`／`ui_order`／`produced_output` の共通契約）。詳細は [PLUGIN_REFERENCE.md](PLUGIN_REFERENCE.md#pluginsbasepy) | — |
...
| `plugins/macro_ensemble.py` | バックエンド | **M-4** 兄弟μ̂スタッキング（ADR-0015・基底 M-1+M-2+M-6 を `BASE_MODELS` 定数駆動・二段ウォークフォワード NNLS）。producer＝`macro_ensemble_scores`・`heavy=True`・`ui_order=370`。詳細は [PLUGIN_REFERENCE.md](PLUGIN_REFERENCE.md#pluginsmacro_ensemblepy) | plugins/macro_snapshots.py, plugins/macro_gbdt.py, plugins/macro_risk_return.py, scipy, xgboost |
| `plugins/macro_gbdt_rank.py` | バックエンド | **M-5** マクロ×財務 ランク学習（ADR-0017・M-2 を継承し4フックのみ override）。**producer なし＝OOF 比較専用**・`heavy=True`・`ui_order=380`。詳細は [PLUGIN_REFERENCE.md](PLUGIN_REFERENCE.md#pluginsmacro_gbdt_rankpy) | plugins/macro_gbdt.py, plugins/utils.py, xgboost, shap |
