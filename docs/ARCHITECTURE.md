...
5. 単一チョークポイント `record_prices_batch`（database.py）で **①daily upsert → ②触れた週のみ daily から weekly を再集約 upsert（aggregate_weeks）→ ③daily を直近 `DAILY_WINDOW_DAYS` で trim**。株価を書く経路（J-Quants/Yahoo、撤去予定の stooq・#736）はすべてここを通る。専用スケジューラ不要（収集に相乗り）
...
+ batch（夜間/月次/watchdog の足跡・batch_freshness.summarize と共有）#563"]
    ...
    subgraph COLLECT["📦 収集管理 /api/collect/（GET 以外は閲覧専用の環境で一律 403・ルーター依存・#733）"]
        ...
        C10["POST /api/collect/history/start\n株価履歴収集を開始（stooq・取得不可。\n画面からは外した・C10〜C13 は撤去予定・#736）"]
...
| `collector_utils.py` | バックエンド | 収集系モジュール共通の設定定数（EDINET/J-Quants/Yahoo/stooq のレート・並列数・バッチ閾値。stooq は撤去予定の個別銘柄経路（#736）とマクロのフォールバック用）とロガー `log`。価格スケール突合の共有定義（丸め許容 `rounding_tolerance`・公式 `AdjFactor` が持たないスピンオフ調整の登録表 `SPINOFF_ADJUSTMENTS` / `spinoff_factor`・#568）もここに置き、検出器と修復スクリプトが書き写さずに共有する | dotenv |
