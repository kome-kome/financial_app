...
| `collector.py` | バックエンド | 後方互換 re-export 層（オーケストレータ）。すべての公開シンボルを下記4モジュールから再エクスポートする。`python collector.py` の CLI エントリポイントを保持 | collector_utils, collector_master, collector_prices, collector_financials |
| `collector_utils.py` | バックエンド | 純粋関数・低レベルヘルパー。XBRLパース（`parse_xbrl_csv`/`parse_raw_rows`）・派生指標算出（`calc_derived`）・株価バリュエーション書き込み（`_apply_price_to_record`）・TSE業種コードマップ（`TSE_INDUSTRY`）・XBRL_MAP（`build_xbrl_map` 逆引き）等 | database.py |
| `collector_master.py` | バックエンド | EDINETコードリスト取得（`fetch_edinet_code_list`）・JPX上場会社一覧から業種補完（`update_industry_from_jpx`） | EDINET API, JPX |
| `collector_prices.py` | バックエンド | 株価収集・市場データ更新。stooq（`collect_stock_price_history`）/ J-Quants（`collect_stock_price_history_jquants`）/ Yahoo Finance（`backfill_historical_stock_prices_yahoo`・`fill_recent_stock_price_gap_yahoo`）の3経路。マクロデータ（`collect_macro_data`・`MACRO_SERIES`：為替・金利・指数・コモディティ9系列） | J-Quants, stooq, Yahoo Finance |
| `collector_financials.py` | バックエンド | XBRL財務収集・CF補完・reparse。フル収集（`run_full_collection`）・差分（`collect_doc_ids_for_period`）・CF補完（`refill_cf_from_xbrl`）・PL/BS補完（`refill_pl_bs_from_xbrl`）・再解析（`reparse_from_raw`）。株価は J-Quants が主経路（stooq は Azure IP ブロックのためローカル補助のみ） | EDINET API, collector_utils, collector_master |
