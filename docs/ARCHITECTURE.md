...
| `plugins/recommend.py` | バックエンド | 複合スコアによる銘柄推薦（VIEW 8指標＋`RUNTIME_METRICS`＝z_momentum/mu）。**μ̂ は opt-in・既定 OFF**（ADR-0030）・`SELECT_COLS` で転送列を限定・`ui_order=110`。詳細は [PLUGIN_REFERENCE.md](PLUGIN_REFERENCE.md#pluginsrecommendpy) | plugins/utils.py, database.py |
...
| `recommend_factor_premia.py` | GitHub Actions / ユーティリティ | recommend の Fama-MacBeth 断面回帰CLI（ADR-0008・Issue #271）。月末スナップショットごとの横断面OLS（`build_period_panel`が`plugins/macro_snapshots.py::build_snapshots`を無改修で再利用・M-1/M-2/M-3と目的変数/母集団/fold共有）→係数の時系列平均をNewey-West（HAC）標準誤差付きで算出（`fama_macbeth_regression`）→`--persist`で`recommend_factor_premia`テーブルへ永続化。第1段階は `--estimator ols|ridge` で切替可（既定 ols＝永続化される重みは従来どおり。ridge は共線性の比較・診断用で `--persist` と併用不可＝昇格ゲート未通過の重みがプリセットへ入るのを防ぐ・#469/ADR-0028）。`plugins/recommend.py::resolve_weights()`が読み「統計的最適化」プリセットとして提供。実行は `recommend-factor-premia.yml`（workflow_dispatch 手動・Issue #342。macro_beta_inference と同型の producer/consumer 分離を Actions 経由でも記録可能に）とローカルCLIの両方 | database.py, plugins/macro_snapshots.py, plugins/recommend.py |
