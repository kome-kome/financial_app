...
| `nightly_scores.py` | GitHub Actions / ユーティリティ | 夜間スコア更新バッチ（Issue #432・親 #423）。`daily-incremental` 成功後に `nightly-scores.yml` から起動され、`NIGHTLY_MODELS`（現状 `sector_ols` のみ・M-6/M-2 は #423 子2 で追加）を `plugins.execute_plugin` 経由で順に回して producer 出力（`regression_results` の `predicted_market_cap`/`gap_ratio`）を更新する。`NIGHTLY_PARAMS` は `params_schema` の default から変えるキーだけを持つ（`sector_ols` は `regularization=ridge`）。**1モデルの失敗は他を巻き込まず**、全実行後に失敗があれば非ゼロ終了（→ `notify-failure.yml` が Issue 起票）。実行ごとに `VERIFIERS` が DB を直接クエリし、プロセス開始時刻より新しい書き込みがあることを確認する（例外なし＝コミット済みとしない） | database.py, plugins/__init__.py, plugins/sector_ols.py |
...
| `.github/workflows/nightly-scores.yml` | GitHub Actions | 夜間スコア更新（Issue #432・親 #423）。`daily-incremental` が **success** で終わった直後の `workflow_run` チェーンで起動し `nightly_scores.py` を実行（＋`workflow_dispatch`）。`workflows:` は `["**"]` 固定で対象は job の `if` の名前一致（`[定常]` の角括弧はフィルタパターンとして壊れる） | `nightly_scores.py` |
