...
graph LR
    ...
    subgraph ANALYSIS["📊 分析 /api/"]
        ...
        AN12["POST /api/backtest/model-comparison\n将来リターン予測 M-1/M-2/M-3/M-4/M-5 の OOF 横並び比較\n（各モデルの oof_backtest を集約・as-of上位Nとは別手法）\n5モデルとも heavy＝ローカル専用（Renderは各モデルskip）"]
    end
...
| `model_comparison.py` | バックエンド | M-1〜M-5 の `oof_backtest` を横並び集約（OOF-first・`POST /api/backtest/model-comparison`）。`tuning_objective_only`/`tuning_dry_run` で重い全社スコアを省く。**全モデルペアの rank-IC 差有意性 `significance_matrix` を併算**（#369） | plugins/, model_stats.py |
