...
| `collector_interim.py` | バックエンド | 半期(H1)財務収集（Issue #219② フェーズB）。EDINET 半期報告書（新式 formCode=043A00/docType160）と旧四半期報告書（043000/docType140）の **Q2(中間=H1累計)** を収集し `financial_records` に `period_type='H1'` で保存。**通期(annual)収集とは独立**・低レベルヘルパ（`fetch_xbrl_csv`/`parse_xbrl_csv`/`calc_derived`/`upsert_financial`）を再利用。新旧いずれも P/L・CF は context `CurrentYTDDuration`(H1累計)・BS は `CurrentQuarterInstant` で既存パーサが無改修で抽出。真の H1 期末は DEI `CurrentPeriodEndDateDEI`（metadata periodEnd は新式でFY末を返すため不使用）、H1 判定は DEI `TypeOfCurrentPeriodDEI=Q2`（旧四半期の Q1/Q3 を除外）。旧四半期は metadata periodEnd 月で事前選別しダウンロード数を削減（`build_fy_end_month_map`/`prefilter_interim_docs`）。CLI `python collector.py --interim --years N`（常に差分）・手動WF `collect-interim.yml` | EDINET API, collector_utils, collector_financials, database.py |
