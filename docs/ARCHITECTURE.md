...
| `.github/workflows/vacuum-maintenance.yml` | GitHub Actions | `stock_price_daily` / `stock_price_weekly` の bloat 対策の週次 VACUUM FULL 自動化（Issue #290）。`_pipeline_vacuum.py` を起動 | `_pipeline_vacuum.py` |
...
| `.github/workflows/bench-macro-beta.yml` | GitHub Actions | Issue #512 の**基準側**（Linux ランナーでの macro_beta 所要）を取る計測ジョブ（workflow_dispatch 専用・定期実行なし）。`scripts/bench_macro_beta.py --mode synth` を回す＝**DB も secrets も使わない**ので Egress ゼロ・`FINAPP_DB_TARGET` 非依存（実データを使うと #503 反転後は Supabase の 2026-08-07 断面とローカル正本で規模が違い、比が無意味になる）。結果 JSONL をアーティファクトで持ち帰り、ローカル実行値と突き合わせる | `scripts/bench_macro_beta.py` |
