...
| `plugins/macro_risk_return.py` | バックエンド | **M-1** マクロ×リスク-リターン推奨（交差項OLS+BIC選択+Walk-forward CV）。producer＝共有 `macro_beta`・`heavy=True`・`ui_order=330`。詳細は [PLUGIN_REFERENCE.md](PLUGIN_REFERENCE.md#pluginsmacro_risk_returnpy) | plugins/utils.py, macro_snapshots.py |
...
| `recommend_factor_premia.py` | ローカル月次バッチ / ユーティリティ | recommend の Fama-MacBeth 断面回帰CLI（ADR-0008・Issue #271）。月末スナップショットごとの横断面OLS（`build_period_panel`が`plugins/macro_snapshots.py::build_snapshots`を無改修で再利用・M-1/M-2/M-3と目的変数/母集団/fold共有）→係数の時系列平均をNewey-West（HAC）標準誤差付きで算出（`fama_macbeth_regression`）→`--persist`で`recommend_factor_premia`テーブルへ永続化。第1段階は **両 estimator とも期内 winsorize→標準化してから解く**（`fit_feature_columns`・#509。係数の単位は「1sd あたり」）。`--estimator ols|ridge` で切替可（既定 ols。ridge は L2 の寄与を測る比較・診断用で `--persist` と併用不可＝昇格ゲート未通過の重みがプリセットへ入るのを防ぐ・#469/ADR-0028）。`plugins/recommend.py::resolve_weights()`が読み「統計的最適化」プリセットとして提供（**`preprocess_version` が一致する行だけ**＝旧単位の重みが新単位の特徴量へ黙って掛かるのを構造的に止める・#517/ADR-0039）。**実行は `scripts/run_monthly.py` の `factor_premia` ステップ**（#503 で cron を停止し #504 で `recommend-factor-premia.yml` を削除。CLI 既定との一致は `tests/test_run_monthly.py` が引き継いだ）。実測 2.6分と軽い | database.py, plugins/macro_snapshots.py, plugins/recommend.py |
