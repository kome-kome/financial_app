...
| `plugins/macro_ensemble.py` | バックエンド | M-4 兄弟μ̂スタッキング・アンサンブル（#367・ADR-0015）。M-1（BIC+OLS）と M-2（`_make_xgb_fit_predict`）の per-(ym,銘柄) OOF を `build_snapshots(return_stock_ids=True)`＋`walk_forward_cv_monthly(return_residuals=True, embargo_months=12)` で自前再現し、(ym, edinet_code) の intersection を母集団に**二段ウォークフォワード**（月 t の統合重みは t 未満の共通 OOF だけで NNLS 学習＝無リーク・`_stack_walk_forward`）で統合。統合残差を共有 `oof_backtest` に通し model_comparison に M-4 として並ぶ。現在μ̂は M-1 `_fit_final`/`_score_companies`＋M-2 全データ最終 XGB を最終重みで合成し `macro_ensemble_scores` へ全置換永続化（producer・`tuning_dry_run` no-op）。`read_producer_scores` は M-2 と同一形＝sell_ranking の `mu_source=macro_ensemble` で利用可。初版は M-1+M-2 のみ（M-3 は週次専用・ADR-0012 で除外）。`SCORING_SOURCES`（as-of VIEW スコア用）は対象外＝メタ検証は oof_backtest＋COMPARISON_MODELS で充足。`heavy=True`・`ui_order=370`（M-3 の後・#378） | plugins/macro_snapshots.py, plugins/macro_gbdt.py, plugins/macro_risk_return.py, scipy, xgboost |
