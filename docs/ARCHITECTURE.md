...
| `model_comparison.py` | バックエンド | M-1〜M-6 の `oof_backtest` を横並び集約（OOF-first・`POST /api/backtest/model-comparison`）。**モデル単位で `progress.emit` する**（#593）——共通骨格（macro_snapshots）の進捗はそのまま流れるが、それだけでは「3本のどれを回しているか」が出ない。終端（`current == total`）も必ず流す＝出さないと画面は「2/3 のまま完了」に見える。`tuning_objective_only`/`tuning_dry_run` で重い全社スコアを省く。**全モデルペアの rank-IC 差有意性 `significance_matrix` を併算**（#369） | plugins/, model_stats.py |
| `model_stats.py` | バックエンド | モデル比較の統計後処理（#369）。定常ブートストラップ（Politis-Romano 1994）で per-fold rank-IC 差の有意性を検定（`significance_matrix`）、分位単調性を要約（`monotonicity_summary`）。stdlib のみ・追加学習/Egress ゼロ・seed 固定で決定的。`oof_backtest`（分位単調性）と `model_comparison`（有意差）が共用 | — |
