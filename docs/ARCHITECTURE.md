...
| `run_local.ps1` | ユーティリティ | `launch.py` を **ローカル読取レプリカ始まり**で起動する PowerShell ショートカット（Supabase 障害中の常用導線）。`FINAPP_DB_TARGET=local` を自プロセスにだけ立ててから起動するので `.env` を書き換えず、復旧後は素の `python launch.py` に戻すだけで prod へ復帰する。起動前にローカルDBへ疎通＋`companies` 件数と週次株価の最新週を表示し、繋がらなければランチャーを起こす前に落とす。`FINAPP_EGRESS_ENFORCE=0` / `FINAPP_EGRESS_LEDGER=0` も併せて立てる（ローカル読取は Egress を消費しないため）。`-Console` でランチャー無しの uvicorn 直起動。本番・CI からは未参照 | launch.py, uvicorn |
