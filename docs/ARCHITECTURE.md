...
| `scripts/backup_push.py` | ユーティリティ | 正本を表ごとに `pg_dump --compress=9` して Supabase Storage へ置く（#503 Phase 3）。**バックアップ元は正本（ローカル）限定**（`guard_source_is_primary`）。ダンプ順は FK 依存の**逆順**（子が先・親が後）＝表ごとに別プロセスでスナップショット時点が揃わないため。保持は直近4世代＋各月の最初を6か月。Free プランの 50MB/ファイル・1GB に対し実測 37.5MB/世代。`--dest local` なら認証情報なしで検証できる | scripts/mirror_common.py, httpx |
