...
| `scripts/batch_common.py` | ユーティリティ | **ローカル駆動バッチの共通骨格**（#504）。`Step` / `Runner` / `BatchSpec` / `run_batch()` と、足跡（`app_settings`）・通知（`gh issue create`）・ログ綴じを持つ。日次と月次は cadence も中身も違うが「走らなかったことを検知する」骨格は同じで、2本にコピーすると片方だけ直す事故が起きるためここが唯一の源。`models_from_steps()` は argv から `--model` を抜き、`HEAVY_AUTOMATION` の照合に使う（列挙を二重に持たない） | database |
...
| `recommend_factor_premia.py` | GitHub Actions / ユーティリティ | recommend の Fama-MacBeth 断面回帰CLI（ADR-0008・Issue #271）。月末スナップショットごとの横断面OLS（`build_period_panel`が`plugins/macro_snapshots.py::build_snapshots`を無改修で再利用・M-1/M-2/M-3と目的変数/母集団/fold共有）→係数の時系列平均をNewey-West（HAC）標準誤差付きで算出（`fama_macbeth_regression`）→`--persist`で`recommend_factor_premia`テーブルへ永続化。第1段階は **両 estimator とも期内 winsorize→標準化してから解く**（`fit_feature_columns`・#509。係数の単位は「1sd あたり」）。`--estimator ols|ridge` で切替可（既定 ols。ridge は L2 の寄与を測る比較・診断用で `--persist` と併用不可＝昇格ゲート未通過の重みがプリセットへ入るのを防ぐ・#469/ADR-0028）。`plugins/recommend.py::resolve_weights()`が読み「統計的最適化」プリセットとして提供。実行は `recommend-factor-premia.yml`（workflow_dispatch 手動・Issue #342。macro_beta_inference と同型の producer/consumer 分離を Actions 経由でも記録可能に）とローカルCLIの両方 | database.py, plugins/macro_snapshots.py, plugins/recommend.py |
...
| `.github/workflows/nightly-scores.yml` | GitHub Actions | 夜間スコア更新＝`sector_ols` + M-6（Issue #432/#443・親 #423）。`daily-incremental` が **success** で終わった直後の `workflow_run` チェーンで起動し `nightly_scores.py` を実行（＋`workflow_dispatch`）。`workflows:` は `["**"]` 固定で対象は job の `if` の名前一致（`[定常]` の角括弧はフィルタパターンとして壊れる）。**リポジトリで唯一 `actions/cache` を使う**（週次株価の run 間キャッシュ・#480／key は run_id でユニーク＋`restore-keys` 前方一致／save は `if: always()`）。Egress 台帳の JSONL（`.egress/*.jsonl`）も artifact に含める（**全ワークフロー共通**・#478 の穴1） | `nightly_scores.py`, `weekly_price_cache.py` |
| `.github/workflows/macro-health.yml` | GitHub Actions | マクロ鮮度ゲート（Issue #420）。`daily-incremental` / `full-pipeline` が **success** で終わった直後の `workflow_run` チェーンで起動し `python -m scripts.check_macro_health` を実行（＋`workflow_dispatch`）。`workflows:` は `["**"]` 固定で対象は job の `if` の名前一致。**このジョブだけが failure になる**設計（収集本体は落とさない） | `scripts/check_macro_health.py` |
| `.github/workflows/recommend-factor-premia.yml` | GitHub Actions | recommend の Fama-MacBeth ファクタープレミアム算出（**毎月5日 UTC 12:00 自動**＋手動・Issue #342／#423 子5）。`recommend_factor_premia.py --persist` を起動。schedule 起動では `github.event.inputs.*` が空になるため run: 側の `\|\| '既定値'` が必須（不変条件は `tests/test_recommend_factor_premia_workflow.py` が強制） | `recommend_factor_premia.py` |
