...
| `.github/workflows/notify-failure.yml` | GitHub Actions | 他ワークフローの `failure`／`cancelled` を Issue へ自動起票（`workflow_run`・Issue #414）。`ci.yml` 以外の全ワークフローを `workflows:` に列挙し、同一タイトルの open Issue があればコメント追記。列挙漏れは `tests/test_workflow_failure_notification.py` が検知 | GitHub Issues（`gh issue create`／`comment`） |
