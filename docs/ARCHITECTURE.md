...
| `model_comparison.py` | バックエンド | M-1〜M-6 の `oof_backtest` を横並び集約（OOF-first・`POST /api/backtest/model-comparison`）。`tuning_objective_only`/`tuning_dry_run` で重い全社スコアを省く。**全モデルペアの rank-IC 差有意性 `significance_matrix` を併算**（#369） | plugins/, model_stats.py |
...
| `plugins/progress.py` | バックエンド | heavy プラグイン実行の進捗を画面へ流す唯一の経路（#545）。`emit()` / `progress_sink()`（ContextVar・`asyncio.to_thread` を跨いで伝播）＋ `PROGRESS_COVERAGE`（heavy ごとの `common`/`own`/`exempt: 理由`）。**sink 未設定なら完全な no-op** ＝月次バッチ・`/api/recommend` 経路は素通り。進捗は必ず「ステップ名」を持ち、数えられる場面では「処理済み/全体」も持つ（**heartbeat は生存を示すが進行を示さない**・#504）。表と実体の照合は `tests/test_plugin_progress.py` | — |
