...
| `recommend_factor_premia.py` | GitHub Actions / ユーティリティ | recommend の Fama-MacBeth 断面回帰CLI（ADR-0008・Issue #271）。月末スナップショットごとの横断面OLS（`build_period_panel`が`plugins/macro_snapshots.py::build_snapshots`を無改修で再利用・M-1/M-2/M-3と目的変数/母集団/fold共有）→係数の時系列平均をNewey-West（HAC）標準誤差付きで算出（`fama_macbeth_regression`）→`--persist`で`recommend_factor_premia`テーブルへ永続化。第1段階は **両 estimator とも期内 winsorize→標準化してから解く**（`fit_feature_columns`・#509。係数の単位は「1sd あたり」）。`--estimator ols|ridge` で切替可（既定 ols。ridge は L2 の寄与を測る比較・診断用で `--persist` と併用不可＝昇格ゲート未通過の重みがプリセットへ入るのを防ぐ・#469/ADR-0028）。`plugins/recommend.py::resolve_weights()`が読み「統計的最適化」プリセットとして提供（**`preprocess_version` が一致する行だけ**＝旧単位の重みが新単位の特徴量へ黙って掛かるのを構造的に止める・#517/ADR-0039）。**実行は `scripts/run_monthly.py` の `factor_premia` ステップ**（#503 で cron を停止＝`recommend-factor-premia.yml` は `workflow_dispatch` の口だけ残る）。実測 2.6分と軽い | database.py, plugins/macro_snapshots.py, plugins/recommend.py |
...
| `.github/workflows/macro-beta-inference.yml` | GitHub Actions | M-1 per-stock 階層マクロβ推論バッチ（Issue #341）。`macro_beta_inference.py` を起動。**schedule は #503 でコメントアウト＝停止中**（現在の駆動は `scripts/run_monthly.py`）。手動起動の口としては生きているが、**GHA からはローカル正本の DB へ書けない**ことに注意 | `macro_beta_inference.py` |
| `.github/workflows/tune-hyperparameters.yml` | GitHub Actions | M-1/M-2/M-3 ハイパーパラメータ探索（Issue #292）。`hyperparameter_search.py` を起動。**schedule は #503 でコメントアウト＝停止中**（現在の駆動は `scripts/run_monthly.py` の `tune:*` 3本） | `hyperparameter_search.py` |
...
| `.github/workflows/recommend-factor-premia.yml` | GitHub Actions | recommend の Fama-MacBeth ファクタープレミアム算出（Issue #342／#423 子5）。`recommend_factor_premia.py --persist` を起動。**schedule は #503 でコメントアウト＝停止中**（現在の駆動は `scripts/run_monthly.py` の `factor_premia`）。schedule 起動では `github.event.inputs.*` が空になるため run: 側の `\|\| '既定値'` が必須（不変条件は `tests/test_recommend_factor_premia_workflow.py` が強制） | `recommend_factor_premia.py` |
