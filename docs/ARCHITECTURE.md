...
| `recommend_factor_premia.py` | ユーティリティ | recommend の Fama-MacBeth 断面回帰CLI（ローカル専用・ADR-0008・Issue #271）。月末スナップショットごとの横断面OLS（`build_period_panel`が`plugins/macro_snapshots.py::build_snapshots`を無改修で再利用・M-1/M-2/M-3と目的変数/母集団/fold共有）→係数の時系列平均をNewey-West（HAC）標準誤差付きで算出（`fama_macbeth_regression`）→`--persist`で`recommend_factor_premia`テーブルへ永続化。`plugins/recommend.py::resolve_weights()`が読み「統計的最適化」プリセットとして提供 | database.py, plugins/macro_snapshots.py, plugins/recommend.py |
