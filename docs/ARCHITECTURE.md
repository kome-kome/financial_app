...
| `feature_disclosure.py` | バックエンド | `statement_disclosure` の予想対比サプライズ等の特徴量化（Issue #322 改善案3・UKI型モデル移植 `build_disclosure_features`/`dedupe_disclosures`/`load_disclosure_features`）。実績(`r_*`)は `cur_per_en` 変化点で単四半期化（1Qは累積=単四半期）、水準の予想(`f_*`)は `cur_fy_en`（年度）単位でのみ更新し年度内据え置き、比率(`pm*`/`cost*`)はその開示時点の生の予想値を使用（水準特徴量とは別扱い）。`m_*`＝実績単四半期−前回開示時点の予想、`d_f_*`＝予想系列の前回開示比差分。`EarnForecastRevision`/`DividendForecastRevision`系は基準系列から除外・同一期間重複はkeep-last。**roe/roa/turn/equity_ratio/cf_ratio系はスコープ外**（`/fins/summary`に四半期BS/CFが無く、四半期粒度データはJ-Quants有料化とセットの#219待ち）。モデルへの組み込み（イベント駆動予測モデル等）は別Issue #323 | database.py（`StatementDisclosure`）, pandas |
