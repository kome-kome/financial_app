...
| `hyperparameter_search.py` | GitHub Actions / ユーティリティ | M-1/M-2/M-3 ハイパーパラメータ自動探索（ADR-0007・Issue #264/#291/#292/#293）。CLI（`--model`＋`--persist`/`--persist-scores`）と月次バッチ（`scripts/run_monthly.py` の `tune:*` 3本）が共有ロジック `run_search()` を呼ぶ（GUIからの手動トリガーはIssue #293で廃止・#292へ一本化）。**`tune-hyperparameters.yml` の cron は #503 で停止**＝現在の駆動はローカルのタスクスケジューラ経由。`tuning_search_space()` を `plugins/tuning.py::search()` で評価し、`persist=True` で best params を `plugin_tuned_params` へ永続化、`persist_scores=True` 併用で最終 `execute_plugin` を1回実行し producer スコアを実反映する。**品質ゲート（Issue #291）**: `persist=True` 時、`plugin_tuned_params` に既存行があればその `objective_value` と今回の `best_score` を比較し、劣化していれば `upsert_tuned_params`（persist_scores 込み）をスキップして `result["skipped_reason"]` を設定・CLI は非ゼロ終了する（初回＝該当行なしはゲート対象外）。GitHub Actions 月次自動実行を人手レビュー無しで運用しても本番値が悪化しないようにするための仕組み。新規 pip 依存なし（`requirements-inference.txt` 分離は不要） | database.py, plugins/tuning.py |
