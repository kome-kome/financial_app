...
| `plugins/macro_snapshots.py` | バックエンド | M-1/M-2 共有スナップショット構築基盤（ADR-0003 §3）。`_MACRO_MAP`／`FIN_BASE_OPTIONS`／`FIN_LOAD_FIELDS`（`load_data` が financial_metrics VIEW から引く36列＝消費列だけ・#459）／`build_snapshots`／`oof_backtest`／`shared_snapshot_cache()` を集約（プラグインではない）。詳細は [PLUGIN_REFERENCE.md](PLUGIN_REFERENCE.md#pluginsmacro_snapshotspy) | plugins/utils.py |
