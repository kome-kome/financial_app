...
| `hyperparameter_search.py` | ユーティリティ | M-1/M-2/M-3 ハイパーパラメータ自動探索（ローカル専用・ADR-0007・Issue #264/#278）。CLI（`--model`＋`--persist`/`--persist-scores`）と GUI ジョブ（`routers/analysis.py` の `tune-start`）の両方が共有ロジック `run_search()` を呼ぶ（Issue #278 で CLI 専用の `_run()` から抽出）。`tuning_search_space()` を `plugins/tuning.py::search()` で評価し、`persist=True` で best params を `plugin_tuned_params` へ永続化、`persist_scores=True` 併用で最終 `execute_plugin` を1回実行し producer スコアを実反映する。新規 pip 依存なし（`requirements-inference.txt` 分離は不要） | database.py, plugins/tuning.py |
