...
| `plugins/macro_snapshots.py` | バックエンド | M-1/M-2 共有スナップショット構築モジュール（ADR-0003 §3）。`_MACRO_MAP` 正本・`build_snapshots`（`build_interactions`／`macro_nan_ok` フラグ。後者=M-2 専用でマクロ欠損を NaN 保持＝企業を落とさず XGBoost に委ねる）・`load_data`・`preload_macro`・`_realized_vol`・`producer_scores`/`get_producer_scores`・**`oof_backtest`（アウトオブサンプル検証ヘルパ・ADR-0004）** を集約。M-2→M-1 結合ゼロ | plugins/utils.py |
