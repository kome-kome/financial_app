...
| `dashboard.html` | フロントエンド | トップページ・全体サマリー（`/`）。「よく使う」の先頭に朝の推奨（`/morning`）への導線 | api.py |
...
| `batch_freshness.py` | ユーティリティ | バッチ足跡（`app_settings` の `*_last_run`）を**測る**層（#561）。`Watched` / `WATCHED` / `collect()` / `status_of()`。読み手は `scripts/check_batch_freshness.py`（毎日 JST 20:00・起票する側）と `routers/morning.py`（`/api/morning` の鮮度カード）の2人。**watchdog 本体を API から import できない**のが切り出しの理由——あちらは import 時に `load_dotenv()` と `FINAPP_DB_TARGET=local` を実行するので、Render（prod）の接続先を書き換えかねない（#508 と同型で、接続先の食い違いは例外を出さず静かに別の DB を読む）。閾値は `run_*.WINDOW_MIN` から導出し**書き写さない** | scripts/run_nightly.py, scripts/run_monthly.py |
