...
| `_pipeline_gh.py` | GitHub Actions | 全件収集パイプライン（full-pipeline.yml から workflow_dispatch 手動起動）。`--refill-cf` モードで既存レコードの CF NULL 補完（capex・net_change_cash）も可能 | collector.py, database.py |
