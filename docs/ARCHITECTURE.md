...
| `plugins/tuning.py` | バックエンド | M-1/M-2/M-3 共有ハイパーパラメータ自動探索エンジン（ADR-0007・Issue #264/#278）。`SearchDim`（探索軸・`only_if`で条件付き軸を values[0]へ縮退）から grid/random で候補を生成し、各候補を `execute_plugin` でフル実行して `oof_backtest` から目的関数（`rank_ic`/`ic_ir`/`long_short`）スコアを抽出する `search()`。M-2/M-3 の producer 永続化は `database.tuning_dry_run()` で候補評価中のみ抑止。`on_progress`/`cancel_check`（省略可）で GUI ジョブ化（`/api/plugins/{name}/tune-start`）向けの進捗通知・中断に対応（既定 None・CLI 挙動は無変更） | plugins/utils.py, plugins/__init__.py, database.py |
