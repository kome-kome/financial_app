...
| `launch.py` | ユーティリティ | Windows ローカル開発用 tkinter ランチャー（uvicorn 起動 GUI）。ポート8000占有者を `/health` の `{"db": ...}` 応答で本アプリか識別し、別アプリ占有時は 8001〜 の空きポートへ退避起動。本番・CI からは未参照の独立ツール | uvicorn |
