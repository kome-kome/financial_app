...
λ はリスク回避度（スライダー、0〜3、既定 1.0）。$R_{\text{axis}}$ は `risk_axis` で選んだリスク（R2 既定 / R1 / R3）。U でランキングし上位 `top_n` 社を返す。

Pareto 最優解（同リスクで高リターン、または同リターンで低リスク）を `is_pareto=True` でフラグし、散布図上で強調描画する。**Pareto 判定軸は表示中の `risk_axis` に追従**し、可視化（横軸）・効用・フロンティア強調を一貫させる（選択軸が None の銘柄は R2 にフォールバック）。
