...
- モメンタム（`use_momentum`、既定 OFF）: M-1 と同一。**M-2/M-6 で ON/OFF を honest OOF 実測した結果、共通 (ym,ec) 域では4検定すべて補正後 α を通らず符号も負のため既定 OFF を維持**（`python -m scripts.momentum_gate`・ADR-0045）
