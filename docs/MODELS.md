...
- 株数は `issued_shares`（XBRL 直接値）を優先し、推計（`bs_total_equity ÷ bs_bps`）は欠損時のフォールバックだけ。推計は IFRS/JGAAP 定義差、期中増資、優先株・転換社債存在時に誤差が生じる
...
「仮定・限界」（本節末尾）を参照（#513）。**成長重視だけは walk-forward で推定した重みを反映済み**
（#546・[ADR-0059](adr/0059-preset-weights-are-re-estimated-walk-forward-keeping-their-order.md) 追記。
推定の比 2.504 : 0.449 を分析画面スライダーの 0.1 刻みに乗せて 2.8 : 0.5）。
...
- **`mu` に重みを付けるときは `mu_source`（M-1/M-3/M-6）を必ず指定する**。未指定なら
...
問わず負（いずれも #546 で成長重視の重みを推定値へ差し替える前の値）。手動プリセットの重みが是正後のスケールに
合っていない件は #546 で扱い、OOF で補正後 α を通った成長重視だけを反映した（ADR-0059 追記）。
...
- **4プリセットは「意味の分かる配分」であって性能最適ではない**（#513）。バランス型・割安重視・
  高収益重視のウェイトは直感的で、rank-IC を目的に推定した値ではない（「統計的最適化」プリセットは
  Fama-MacBeth 断面回帰によるデータ駆動）。**成長重視だけは #546 で walk-forward 推定の重みへ差し替えた**
  （大小順を保って rank-IC を直接高める推定・OOF で補正後 α を通過。下記）。
  61期パネルの実測（#509・[ADR-0008](adr/0008-recommend-factor-premia-fama-macbeth.md)・成長重視の差し替え前）では
  **rank-IC が正なのは成長重視（+0.0605）とバランス型（+0.0210）だけ**で、割安重視
  ...
  重みの再設計そのものは #546 で扱う。**推定し直す道具は `python -m scripts.preset_weight_walkforward`**
  （#625・[ADR-0059](adr/0059-preset-weights-are-re-estimated-walk-forward-keeping-their-order.md)）で、
  学習期間の rank-IC を直接高める重みを**静的重みの大小順（プリセットの性格）を保ったまま**月ごとに推定し、
  embargo 12か月を空けた OOF の rank-IC を同じ月の静的重みと対にして検定する。**`PRESETS` はこの比較で
  補正後 α を通ったものだけ変える**（最終重みを `preset_ic_gate` で測る比較は in-sample なので根拠にしない）。
  2026-09-19 の62期（OOF 32か月・α=0.0125）で通ったのは**成長重視だけ**（推定 +0.1470 対 旧静的 +0.1113・
  32/32か月で上回る）で、#546 で反映した。性格は「売上が主軸・残り4指標は同格」に変わった（32 fold すべてで
  下位4指標が同じ重みに張り付いた）。バランス型（p=0.198）・割安重視（p=0.047）・高収益重視（p=0.168）は ns で静的のまま。
  反映値は画面のスライダー刻みに乗せる（乗らない値は分析画面経由だけ黙って丸められる・`tests/test_recommend.py` が照合）。
...
企業固有の財務指標の OLS で1年先リターン μ を推定し、各銘柄をリスク-リターン平面に配置して**効率的フロンティア**（Pareto 最優解）を提示するモデル。マクロ要因（為替・金利・株式市況）とその交差項は `use_macro` で足せるが、**既定は OFF**（#615・ADR-0050 の 2026-09-20 追記＝共通 (ym,ec) 域の実測で rank-IC を有意に下げたため）。**モデルからマクロが消えたわけではない**——マクロ起因リスク R_macro（§9.5）は `risk_axis` で選べるまま残り、`macro_beta` の producer 経路で別に入る。
...
> **設計判断（2026-06-19）**: 旧「貪欲前進BIC＋VIF」は 36,000 行規模で OLS を約1.2万回呼び、`use_macro=true`（当時の既定）で分単位を要した。LassoLarsIC への置換で特徴量選択は秒未満に短縮（実測：選択フェーズ 0.7s）。非劣位チェック（`use_macro=false` 構成）で旧/新の walk-forward CV mean R² は同値（0.0122）を確認済み。詳細は §9.10。

### 9.4 Walk-forward CV

既存の `walk_forward_cv_monthly`（`plugins/utils.py`）で月次ロールウィンドウ CV を実施。時系列順を厳守（通常の k-fold はルックアヘッドバイアスが生じるため不可）。各フォールドの RMSE・MAE・R² を記録。**学習サンプルが要求する履歴長は特徴量構成で決まる**：52週先リターン（未来）は常に必要だが、**12ヶ月モメンタムは `use_momentum=ON` のときだけ過去履歴を要求する**。`use_momentum=OFF`（既定）なら `use_macro=ON` にしても過去履歴要件が外れ、週次株価が浅くてもフォールドが確保できる（§9.8）。**現在は週次が7年分あるので ON でも fold は立つ**（実測 15 fold）。既定が OFF のままなのは履歴不足ではなく、ON/OFF の OOF 実測で効果が無かったため（§9.8-4・ADR-0045）。
...
| 構成 | 旧 | 新 | 備考 |
|---|---|---|---|
| `use_macro=true`（**当時の**既定・fin4・max20） | 約219s | **約29s** | 主因は特徴量選択ではなく `_build_snapshots` のマクロ計算（企業×月の全日付再走査）。マクロ特徴量は snap_date のみに依存するため**日付メモ化**で 207.9s→12.7s。 |
| 特徴量選択フェーズ単体 | 約1.2万回 OLS | **0.7s** | LassoLarsIC の1パス LARS（§9.3）。 |
| `use_macro=false`（非劣位検証・**#615 以降の既定**） | mean R²=0.0122 | mean R²=0.0122 | 旧/新で同値。選択法置換による CV 品質劣化なし。 |
...
プリセット（`マクロ予測型`（**既定**）/ `バランス型` / `割高警戒型` / `業績悪化重視`）は $w_i$ の既定値、UI でスライダー上書き可（スライダーは既定で折りたたみ表示）。**マクロ予測型**は期待リターン μ（`mu`）とマクロリスク −Rᴹ（`neg_r_macro`）の2軸のみを用いる（スコアは $\sum w_i(-z_i)/\sum w_i$ で正規化されるため両者の**比率のみ有意**・既定 1.0 : 0.5）。μ の出所（`mu_source`）は M-1（`macro_risk_return`）／M-3（`macro_dlm`）／M-6（`macro_enet`・**既定**・#396）から選ぶ（M-4 は退役・ADR-0044／**M-2 は供給者から降ろした**・#572・ADR-0052）。**R3 足切りゲートが効くのは `r1_prime` を持つ M-1・M-6 のみ**（M-3/M-4 は不在＝無効。M-2 は `r1_prime` を持つがトグルから外れたので到達しない）。
...
- マクロ（`macro_features` multiselect）: M-1 と同一選択肢（`use_macro=True/False` で制御）。**既定は M-2 が ON・M-1 が OFF**（#615。選択肢の顔ぶれは同じで、既定だけ割れている）
...
あわせて per-stock μ̂ と `r1_prime` を `macro_gbdt_scores` テーブルへ**全置換で永続化**する（`sector_ols`→`regression_results` と同型・producer.execute 直書き）。かつては**売り候補ランキング（§10）が `mu_source` トグル**で M-2 を選べたが、**2026-09-07 に供給者から降ろした**（#572・ADR-0052）＝トグルの選択肢は M-1／M-3／M-6 のみになった。永続化自体は続く（M-4 が基底として読むため）。M-2 の `read_producer_scores` は M-1 と同一形 `{mu, r_macro, r1_prime}` を返す（`r_macro` は共有 `macro_beta`）。**`r1_prime` は §11.7.1 のコンフォーマル区間半幅**で埋め、**R3 足切りゲートは M-2 選択時も機能する**（#365・ADR-0020。列未 migration / 旧スナップショットの `r1_prime=None` はゲート素通り）。
...
- inner-CV グリッド / optuna によるハイパラ自動探索 → #264〜#267 でwalk-forward OOF
  ...
  **悪化** −0.0264・p=0.001）。測定入口は
  ...
  <br>**M-1・M-2 ともこの2軸を探索空間から外した**（#604）。M-1 は 72 → **12通り**で、
  BIC が `momentum_12m1` を一度も選ばないため**モデルは1ミリも変わらない**。M-2 は
  1,728,000 → **288,000通り**だが探索は random 150件固定なので**所要は変わらず**、同じ件数で
  より良い候補に当たりやすくなる——ただし木モデルは列がそのまま入るので **μ̂ は変わる**
  （向きは実測では改善側）。副次効果として M-2 の `build_snapshots` キャッシュが
  momentum 構成6種から1種へ減る（`_CACHE_MAXSIZE=8` の圧迫が解消）。
  M-5 は M-2 を継承するので同じ変更が波及する（#570 で退役済み）。
  **M-6 は2軸を持ったまま**——今回の窓スキャンで測っておらず外す根拠が無い。実害も無い
  （`TUNE_MATRIX` に入っておらず探索が一度も走っていない）
...
- **μ̂ は夜間バッチが日次更新する**（#443）。M-6 は月次のハイパーパラメータ探索
  （M-1/M-2/M-3）に入っておらず `--persist-scores` の副作用も受けないため、#443 以前は
  ローカル手動実行だけが `macro_enet_scores` の更新経路だった。現在は
  `nightly_scores.py` の `NIGHTLY_MODELS` に載っており、夜間バッチ
  （`run_nightly.ps1`・JST 17:20）が **params_schema の既定構成のまま**（本節の実測と同一）
  自動で回す。tuned params は持たない。
...
| 2026-09-07 | **M-2 を μ̂ の供給者から降ろした（退役ではない）**（#572・ADR-0052）。`mu_source` トグル（`sell_ranking` / `recommend` / `templates/analysis.html` の静的 select）から `macro_gbdt` を除去し、選択肢は M-1／M-3／M-6 になった。**`hidden` は立てない**ので サイドバー・分析タブ・SHAP 表示・`COMPARISON_MODELS` はそのまま動く。根拠は棚卸し——M-4 の基底という支えは #570 の退役で弱まり、M-2 固有だった分割コンフォーマル区間 `r1_prime` は M-6 も持ち（#396）、honest OOF rank-IC では M-6 に有意劣後（0.1419 vs 0.1713・p=0.002・ADR-0021）。残る固有価値は **SHAP による非線形の解釈**と**比較の基準線**の2つで、どちらも `mu_source` を通らない。**月次探索の `--persist-scores` は据え置き**（M-4 の基底として `macro_gbdt_scores` を読むため）＝`HEAVY_AUTOMATION` / `PRODUCER_COVERAGE` / `PROGRESS_COVERAGE` も変更なし |
