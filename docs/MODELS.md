...
現状は Gauss-Jordan 消去法による Pure Python 実装（`plugins/utils.py` の `ols()`）。
ただし VISION.md「サードパーティーライブラリ採用基準」を満たす成熟ライブラリ
（numpy・scipy・statsmodels・scikit-learn 等）への置換は許可されている。
特に統計推論（適切な t/F 分布、ロバスト標準誤差、残差診断）が必要な場面では
`statsmodels.OLS` への移行を推奨する。
...
ここで df = n − p。`df < 30` の場合は t分布の裾の厚さを反映する簡易補正を
適用するが、小サンプルでは「参考値」扱いとする。
