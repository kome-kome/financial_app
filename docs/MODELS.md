...
λ はリスク回避度（スライダー、0〜3、既定 1.0）。$R_{\text{axis}}$ は `risk_axis` で選んだリスク（R2 既定 / R1 / R3）。期待リターンは **μ_raw**（OLS の生予測値。セクター収縮は低シグナル時に銘柄差を消すため廃止）。

**効用 U・Pareto 判定・並べ替え・`top_n` 抽出は λ／リスク軸にのみ依存する後処理であり、モデル再学習に一切関与しない**。そのためサーバー（`_score_companies`）は**全社の raw 値**（`mu_raw / r1 / r2 / r3`）を返し、これら後処理は**クライアント側（`static/js/analysis.js`）で算出**する。結果として λ 調整・リスク軸切替・表示件数変更は**再計算なし（再API なし）で即時反映**される（重い計算が走るのは特徴量・マクロ・`max_features` 等モデル本体のパラメータを変えた時のみ）。Pareto 判定軸は表示中の `risk_axis` に追従する。
