...
$$U = \mu_{\text{shrunk}} - \lambda \cdot R_{\text{axis}}$$

λ はリスク回避度（スライダー、0〜3、既定 1.0）。$R_{\text{axis}}$ は `risk_axis` で選んだリスク（R2 既定 / R1 / R3）。
...
**可視化マッピング**（バブルチャート）: **y=μ_shrunk / x=選択リスク軸（R1/R2/R3・クライアント即時切替） / 色の濃淡=効用 U / バブル径=R1 信頼度 / 枠線強調＋フロンティア線=Pareto 優位**。μ 収縮（§9.6・R1 依存で λ/軸に非依存）はモデル確定値のためサーバー側で算出する。
