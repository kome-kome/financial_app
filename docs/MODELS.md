...
しない）ため、鮮度は cron が担保する。2026-08-08 の実測（**#509 の是正前**）で有効期間 61・
...
- **VIEW 由来の `z_*` は winsorize を通していない**ため、値そのものは大半が 0 近傍へ潰れている
  ...
  **#509 で消費側を是正済み**——`recommend.execute` / `backtest.score_record` は
  `fit_view_metric_stats` が断面ごとに作り直した (mean, sd) で標準化してから加重するので、
  重み 1.0 の実効的な影響力は列間で揃う（是正前は最大 73倍違った＝IQR 比 `z_op_margin` 0.01x ／
  `z_momentum` 1.00x）。**画面に出る指標値（`results[].detail`）は生値のまま**で、
  変わったのはスコアの合成単位だけ。`z_momentum` / `mu` は算出側で期内標準化済みなので対象外。
