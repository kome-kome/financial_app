...
| **4** 🔄 | 導線リンク（各一覧→企業ページ）**実装済み** ／ ツールチップ・レスポンシブ・CSV/画像出力は未着手 |
...
### Phase 4 実装メモ（導線リンク・2026-05-26）
- 各一覧の企業名を `/company/{edinet_code}` への `<a class="co-link">` リンク化（`edinet_code` 無しの行はプレーンテキストにフォールバック）。バックエンド変更なし。
- `collection.html`: スクリーニング表（モーダル呼び出しを撤去しリンクに置換）／DBブラウズ表（リンク追加、「詳細」モーダルは温存）。
- `analysis.html`: 乖離・推薦・総合リターン・ネットキャッシュ・バックテストの5表。
- `db.html`: ドリルダウン結果に「企業ページを開く」リンク追加。あわせて入力検証を `^E\d{6}$` → `^E\d{5,6}$` に修正（実コード〔E+5桁〕が弾かれていた既存不具合を解消）。
- 各テンプレに `.co-link` クラス追加。`<a href>` 同タブ遷移（中クリックで新規タブ可）。
- 未着手: グラフ画像/CSVエクスポート・レスポンシブ仕上げ。
- 検証: 4テンプレの inline JS `node --check` 通過、リンク追加・モーダル温存を grep 確認。**遷移・描画の目視は Render で要実施**。
