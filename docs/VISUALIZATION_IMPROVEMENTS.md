...
| 項目 | 内容 |
|---|---|
| ステータス | 🔄 **Phase 1・2 と F9(同業比較) 実装済み**（F1〜F9）／F10(横断分布) と Phase 4 は提案 |
| 作成日 | 2026-05-26 |
...
| **3** 🔄 | F9（同業比較）**実装済み** ／ F10（横断分布・`analysis.html`）未着手 |
...
- **F10（横断分布・`analysis.html` の散布図/ヒストグラム）は未着手**。`analysis.html` は Chart.js 未読込のため、CDN 追加＋既存SVGの置換が必要。
