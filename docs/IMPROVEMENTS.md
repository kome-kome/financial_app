...
| 番号 | 項目 | 状態 |
|---|---|---|
| P2-4 | `pandas.rolling()` でテクニカル指標高速化 | 未着手 |
| P2-5 | `sklearn.model_selection.TimeSeriesSplit` で時系列 CV 標準化 | 未着手 |
| P2-6 | `sklearn.linear_model.Ridge/ElasticNet` で正則化回帰の選択肢追加 | 未着手 |
| P2-7 | `statsmodels.tsa.ARIMA(1,0,0)` で OU 過程 κ を MLE 推定 | 未着手（FUTURE_TASKS Tier 2-B と統合候補） |
| P2-8 | `numpy.percentile` / `scipy.stats.zscore` への置換 | 未着手（軽微） |
