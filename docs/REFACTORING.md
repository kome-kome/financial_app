...
ローカル DB に既存データがある開発者向けに、Supabase へ移行するワンショットスクリプトを作成する。

```
scripts/migrate_local_to_supabase.py
  ├── pg_dump で localhost からエクスポート
  ├── psql で Supabase にインポート
  └── 件数比較で整合性チェック
```

ただし「**マスターは EDINET（再収集すれば作り直せる）**」のため、移行に失敗してもローカル DB を捨てて Supabase で `python collector.py --years 5` し直せばリカバリ可能。スクリプトは便宜的なもの。
...
| Phase | 内容 | 後方互換 | 想定工数 |
|---|---|---|---|
| **P1** | DB 一本化のドキュメント整備（`CLAUDE.md` / `DEPLOYMENT.md` / `README.md`）| 完全互換 | 半日 |
| **P2** | `XbrlRawDocument` テーブル追加 + `collector.py` で raw 保存。`run_full_collection` に組み込み | **完全互換**（既存 financial_records も同時に書く） | 1 日 |
| **P3** | `reparse_from_raw` の CLI 実装 + 動作確認テスト | 完全互換 | 半日 |
| **P4** | `templates/collection.html` に「再 parse」ボタン + `/api/collect/reparse` エンドポイント追加 | 完全互換 | 半日 |
| **P5** | 既存 Supabase データを全件再収集（raw も生成）| データ更新あり | EDINET レート制限次第 |
| **P6** | `migrate_local_to_supabase.py` スクリプト + ローカル DB 廃止アナウンス | 互換破壊（ローカル DB が使えなくなる）| 半日 |
| **P7** | `financial_records.raw_xbrl_json` カラム drop（drop migration）| 互換破壊（小） | 半日 |
