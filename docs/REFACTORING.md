...
| market_data | 2,000,000 | 1,500,000 | ローカル |
...
pg_dump --no-owner --no-acl --data-only \
  --table=companies --table=financial_records --table=market_data \
  --table=collection_logs --table=auth_state \
  postgresql://edinet:edinet@localhost:5432/financial_db \
  > local_dump.sql

# Supabase へ投入（先に対象テーブルを truncate）
psql "$SUPABASE_DATABASE_URL" <<SQL
TRUNCATE companies, financial_records, market_data, collection_logs, auth_state
...
SELECT MAX(date), MIN(date), COUNT(DISTINCT edinet_code) FROM market_data;
```
