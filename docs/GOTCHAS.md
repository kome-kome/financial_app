...
- **e-Stat CPI 年次行混入問題（解決済み #256）**: statsDataId=0003427113 は月次（1970年〜現在）と年次集計が同一テーブルに混在。年次行の `@time` は `YYYY100000`（10月公表）で先頭6文字が `YYYY10` → month=10 → lag_days=30 シフト後 XXXX-10-31 になり「10月のみ10件」に見えた。**fix**: `fetch_estat_series` に `lvTime=4`（月次階層のみ）を追加。東京都区部（cdArea=13100）も同一 statsDataId 内に存在し、別 ID 不要。
