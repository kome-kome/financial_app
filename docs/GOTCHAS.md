...
- **FRED の OECD 旧系列は凍結（更新停止）があり、採用前に最終更新日を必ず確認する**: 例 月次CPI `CPALTT01JPM657N`（2021/6 で停止）・M2 `MYAGM2JPM189S`（2017/2 で停止）。#250 で採用した4系列（JPNRGDPEXP / LRUNTTTTJPM156S / JPNPROINDMISMEI / XTNTVA01JPQ664S）は現行更新中を ALFRED で確認済み。CPI・M2・短観・個人消費・設備投資等は FRED 凍結/未提供のため e-Stat/日銀コネクタを要する第2弾（別Issue）。
...
- **statsDataId=0003427113 は年次集計（月次ではない）**: 全国 CPI（JP_CPI_TOTAL/JP_CPI_CORE）は 1年1行（10月公表）しか蓄積されない。月次データを収集するには別の statsDataId が必要（要調査・別 Issue）。M-1 YoY 計算は年1点でも動作するが精度は低い。
- **JP_CPI_TOKYO（cdArea=13100）は statsDataId=0003427113 に存在しない**: 東京都区部 CPI は別の statsDataId を使う必要がある（要調査・別 Issue）。現在は毎回「データ無し」でスキップされる（安全弁あり）。
