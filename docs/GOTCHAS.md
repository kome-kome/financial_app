...
- **`raw_xbrl_json` 列は常時保存される（`SKIP_XBRL_RAW` のスコープ外）**: `upsert_financial`（`database.py:794`）は毎回 bs/pl/cf のマップ済み dict を `financial_records.raw_xbrl_json`（JSON 列）へ書き込む。`SKIP_XBRL_RAW=true`（デフォルト）が抑止するのは**別テーブル `xbrl_raw_documents`（生 CSV 行・TOAST で大容量）への書き込みのみ**で、本列はフラグに関係なく常に保存される（名前と列コメント「デバッグ用」に反して、現状この dict は parse 済み値しか持たず生タグは残らない）。**ストレージ影響**: 本列が `financial_records` 約73MB の主因（Supabase 500MB 制約下の第2の容量レバー）。削減したい場合は `SKIP_XBRL_RAW` ではなく **drop マイグレーション**が必要（[DEPLOYMENT.md](DEPLOYMENT.md) 容量設計「後続PR」/ [FUTURE_TASKS.md](FUTURE_TASKS.md) PR-B 参照）。
...
- **FRED の OECD 旧系列は凍結（更新停止）があり、採用前に最終更新日を必ず確認する**: 例 月次CPI `CPALTT01JPM657N`（2021/6 で停止）・M2 `MYAGM2JPM189S`（2017/2 で停止）。#250 で採用した4系列のうち `JPNPROINDMISMEI`（鉱工業生産）は **2024-04-30 で凍結が確認された（#253）**。e-Stat コネクタ実装まで JP_IP を除外中。残存3系列（JPNRGDPEXP / LRUNTTTTJPM156S / XTNTVA01JPQ664S）は引き続き更新中を確認済み。CPI・M2・短観・個人消費・設備投資等は FRED 凍結/未提供のため e-Stat/日銀コネクタを要する第2弾（別Issue）。
...
- **e-Stat CPI cdCat01 コード（statsDataId=0003427113・2020年基準）**: `0001`=全国総合, `0161`=生鮮食品を除く総合（非季調コア）, 東京都区部は `cdArea=13100` + `cdCat01=0001`。季節調整済みコア `0902` との混同注意（BOJ が政策判断に使うのは非季調コア `JP_CPI_CORE`）。
- **e-Stat CPI 年次行混入問題（調査中 #262・#256 時点の `lvTime=4` fix は撤回済み）**: statsDataId=0003427113 は月次（1970年〜現在）と年次集計が同一テーブルに混在。年次行の `@time` は `YYYY100000`（10月公表）で先頭6文字が `YYYY10` → month=10 → lag_days=30 シフト後 XXXX-10-31 になり「10月のみ10件」に見えた。#256 時点では `lvTime=4` の追加で解決したと記録したが、実際には `lvTime=4` で JP_CPI_TOTAL が解析失敗・他系列は0行、次点の `lvTime=2` は全系列解析失敗（HTTP 200 だが `DATA_INF.VALUE` パス自体が無い）となり、**`lvTime` パラメータは撤去して年次10行の安定状態に戻した（#257→#262）**。現在の仮説修正: `cdTab`（表章項目）を全く指定していなかったことが年次のみ返却の原因ではないかとみて `cdTab=1`（指数）を追加済み（未検証・本番/ローカル収集での実データ確認が必要）。東京都区部（cdArea=13100）も同一 statsDataId 内に存在し、別 ID 不要という理解は変わらず。FRED代替は不可：core-CPI 相当の `JPNCPICORMINMEI`/`JPNCPICORQINMEI`（ex food&energy）は2021-06で更新停止しておりそもそも「生鮮食品を除く総合」とは定義も異なる。
