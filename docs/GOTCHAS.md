...
- **コモディティ指数・先物ティッカーの生存確認（#358・ADR-0013）**: 指数ティッカーは配信停止リスクがあるため（`^JGB`/`^TPX` の前例）、追加前に Phase 0 疎通検証（`fetch_yahoo_history` 経由・DB書込なし）を実施。2026-07-20 時点で **総合指数 `^BCOM`（ブルームバーグ商品指数）は生存**（6年 1506 行・5年 1254 件・最新値は直近営業日）、個別先物 `HG=F`（銅）/`NG=F`（天然ガス）/`SI=F`（銀）/`ZW=F`（小麦）/`ZC=F`（トウモロコシ）/`ZS=F`（大豆）/`PL=F`（プラチナ）も全て 6年 1508-1510 行取得可を確認。**stooq はコモディティ先物がローカル IP でも全滅（0件）** のため `MACRO_SERIES` の `ticker` は空文字（Yahoo フォールバック時に安全に skip）。GHA Azure IP 上での `^BCOM` 最終疎通は collect-macro.yml 実行で確認する（#218 の2PR 公開フロー）。
...
- **マクロ系列の収集開始日の差が strict モデルの学習期間を律速する（#379 の派生・#381 で解決）**: strict は全系列同時に非 None を要求するため、**最も新しい系列**が学習可能期間の下限を決める。2026-07-23 時点で `HY_OAS`/`IG_OAS` のみ 2023-06-26 開始（他は 2016-06 開始・コモディティ8系列は 2020-07 開始）で、M-1 strict のスナップショットは **24ヶ月（2023-07〜2025-06）しか立たなかった**。`min_train_months=6` + embargo=12（ADR-0014）では fold が 2 期しか取れず honest OOF の統計的信頼性が低い。
  ...
  - **対策（#381）**: 既定の信用ファクターを非ICE代替 `BAA_SPREAD`（FRED `BAA10Y`＝Moody's Baa 社債利回り−米10年金利・日次・1986〜で truncate されない）へ移行。`HY_OAS`/`IG_OAS` は選択肢としては残す（手動 ON で直近3年窓を使える）が **`DEFAULT_MACRO_FEATURES` からは除外**（`plugins/macro_snapshots.py` の `_STRICT_TRUNCATED_FEATURES`）。これで strict の律速はコモディティ8系列（2020-07 開始・yoy で 2021-07 以降）まで緩む。系列ごとのカバレッジは集約クエリ `SELECT series_code, min(trade_date), max(trade_date) FROM macro_data GROUP BY 1` で確認できる。
