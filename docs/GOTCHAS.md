...
- **マクロ系列の収集開始日の差が strict モデルの学習期間を律速する（#379 の派生）**: strict は全系列同時に非 None を要求するため、**最も新しい系列**が学習可能期間の下限を決める。2026-07-23 時点で `HY_OAS`/`IG_OAS` のみ 2023-06-26 開始（他は 2016-06 開始・コモディティ8系列は 2020-07 開始）で、M-1 strict のスナップショットは **24ヶ月（2023-07〜2025-06）しか立たない**。`min_train_months=6` + embargo=12（ADR-0014）では fold が 2 期しか取れず honest OOF の統計的信頼性が低い。`FRED_MIN_YEARS_BACK=10` は既にコードにあるため**マクロ再収集で backfill 可能**（系列ごとのカバレッジは集約クエリ `SELECT series_code, min(trade_date), max(trade_date) FROM macro_data GROUP BY 1` で確認できる）。
