...
**M-1 マクロ×リスク-リターン推奨**（財務 OLS+`LassoLarsIC(BIC)`選択+OLS再フィット+Walk-forward CV。マクロ特徴量と交差項は `use_macro` で足せるが**既定 OFF**・#615。マクロ起因リスク `r_macro` は横軸として残る）。**全社 raw を返却し JS 後処理**。`heavy=True`・`ui_order=330`。

共有ロジックは `macro_snapshots.py` に移管（ADR-0003）。**`oof_backtest` 結線済み（#272）**・`tuning_search_space()`（`max_features` の単一軸グリッド・**6件**。#265 で 72件 → #604 でモメンタム2軸を外して 12件 → #615 で `use_macro` を外して 6件。`use_momentum` と `use_macro` は `base_params` で False に固定する）。**`min_coverage` は軸にしない**（#596・ADR-0049）——到達行の充足率は常に 1.0（`use_macro=ON` なら `macro_nan_ok=False` でマクロ欠損の断面が充足率チェックの手前で破棄され、既定の OFF ではそもそも NaN 源が無い・#615）（実測: 0.0〜1.0 のどの値でもサンプル総数 181,862 で不変／2026-09-02 の完走ログ288件でも72群すべて群内不変）。軸を足しても失敗として現れないので、**288件のうち216件が同一結果の再計算だった**。`params_schema()` には残す（M-2 では実際に効く）。探索中（`shared_snapshot_cache()`）は異なる `max_features` 候補で BIC 選択結果（`selected_names`）が偶然一致した場合、後続の `walk_forward_cv_monthly()` を再実行せず使い回す（`cv_by_selected_features`・Issue #304。真の重複排除＝近似ではない）。
...
- **`risk_axis`（`r2` 既定 / `r_macro`）は 2026-09-21 に測る手続きだけ入った＝結果はまだ無い**（#709・ADR-0050 の 2026-09-21 追記）。測れていなかったのは**ここまでの測定がすべて μ̂ の順位しか見ていなかった**から——`oof_backtest` は `{ym: [(yhat, y_true)]}` から rank-IC を作り、`yhat` は μ̂ そのものなので `risk_axis` を変えても差が構造的にゼロになる。入口は `python -m scripts.momentum_gate --risk-axis`（日中枠 `gate:risk-axis`）で、3条件 `mu_only` / `r2`（分母） / `r_macro` を**同一パネル・同一 CV**（`Cond` が同一なので memo で1回に畳む＝μ̂ をビット単位で一致させる）で比べ、R を月内 z-score 標準化してから `U = μ − λ·z(R)` を作る。**`r_macro` は時点再現できない**（`macro_beta_loadings` に時点列が無く run は 2026-07 以降の数本＝評価する全ての月に後のデータで推定した β を当てる）ので、結果は**未来情報を含む上限**として読む——負ければ棄却、勝っても既定を変える根拠にはならない。副作用で `r_macro` は銘柄ごとの定数になり、この軸は「マクロか実現ボラか」と「静的か動的か」を同時に動かす。
...
- **M-1・M-2 とも2軸を `tuning_search_space()` から外した**（#604・`base_params` で `use_momentum=False` を明示固定）。M-1 は 72 → **12通り**、M-2 は 1,728,000 → **288,000通り**（ただし random 150件固定なので所要は変わらず、同じ件数で良い候補に当たりやすくなる）。**M-2 は木モデルなので μ̂ が変わる**（M-1 は BIC が選ばないので変わらない）。M-2 では副次効果として `build_snapshots` のキャッシュが momentum 構成6種（off＋窓5種）から**1種**へ減り、`_CACHE_MAXSIZE=8` の圧迫が解ける——**軸を戻すとここも元に戻る**ので、戻すなら `_CACHE_MAXSIZE` との関係を測り直すこと（#298・#588）。**M-5 は M-2 を継承するので波及する**（#570 で退役済みだが穴は同じ）。**M-6 は2軸を残している**（窓を測っていない＝外す根拠が無い。`TUNE_MATRIX` に無く探索が走っていないので実害も無い）。
- **保存済みの調整値は `project_tuned_params()` が現在の探索空間へ射影してから画面へ出す**（#604）。`plugin_tuned_params` は「そのとき探索した空間」の記録なので、軸を外すと古い値が「🔧 自動調整済み」としてプリフィルされ続ける。射影は `base_params` のキーをその値で上書き・dims の軸は保存値のまま・どちらにも無いキーは落として既定へ倒す。**黙って変えない**（生値 `params_as_tuned`・変更キー `stale_params` を返し画面が注記）。**DB は書き換えない**（保存値は実際の探索結果の記録＝監査用）。
- **射影は探索空間の形しか見ないので、パネルの世代は `panel_changed` が別に見る**（#711・ADR-0047）。`current_panel_fingerprint()`（規則の唯一の源は `hyperparameter_search._data_fingerprint()`・TTL 60秒キャッシュ）と保存時の `data_fingerprint` を比べ、`True`=違う / `False`=同じ / `None`=判定不能を返す。画面は **`False` と積極的に言えたときだけ**自動プリフィルし、それ以外はバッジの文言を「前回の探索結果」に落として理由を注記する（ボタンからの手動適用は残す）。**パネルは毎晩伸びるので自動適用は事実上探索当日しか働かない**——これが無いと、いま探索中の軸に残った古い値が素通りする（M-1 の `max_features=5` が実際にそうで、rank-IC +0.0052・fold 間 std 0＝予測値が月内で全銘柄同じだった）。
