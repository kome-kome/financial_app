...
>
> **再有効化の段階運用（2026-06-10〜）**: `.yml` には復帰済みだが、`schedule`（cron）は**実証までコメントアウト**している。理由＝(a) dual-table 移行(2026-06-06)後このワークフローは一度も新スキーマ下で走っていない、(b) 鮮度を担う Yahoo ギャップ補完が GitHub Actions の Azure IP から到達できるか未実証。**手順: まず `workflow_dispatch` で手動1回実行 → 株価が当日付近まで前進し Yahoo 補完が成功したことを確認 → `on.schedule` のコメントを外して cron を再開**する。
...
**M-1 特徴量への公開手順（本番蓄積確認後）**:
1. `collect-macro.yml` を `workflow_dispatch` で実行（`FRED_API_KEY` シークレット設定済みであること）
2. Supabase で `SELECT series_code, COUNT(*) FROM macro_data WHERE series_code IN ('HY_OAS','IG_OAS','BREAKEVEN10Y','JP10Y_FRED','T10Y2Y') GROUP BY series_code` でレコード数確認
3. `plugins/macro_risk_return.py` の `_MACRO_MAP` と `MACRO_FEATURE_OPTIONS` のコメントアウト（`# FRED チャネル` 行）を解除してコミット
