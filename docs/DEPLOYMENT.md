...
- バックグラウンドのスケジューラー（`api.py:_daily_scheduler`）は
  **スピンダウンすると停止する**。現状の運用方針:
  - **収集は GitHub Actions に統一**: `.github/workflows/full-pipeline.yml` を `workflow_dispatch`
    で手動起動する設計。Render Free のメモリ/タイムアウト制約を回避し、確実に完走させる
  - 過去の補完策（`_startup_catchup` と keepalive ワークフロー）は廃止済み
  - **将来の常時稼働が必要な場合**: 有料プラン ($7/月) または Render Cron Jobs (別 service として定義)
...
収集は `.github/workflows/full-pipeline.yml` を `workflow_dispatch` で手動起動する設計に統一
したため、Render を常時起動させておく必要がなくなった。アプリにユーザーアクセスがあった
...
どちらも Render Free のメモリ/タイムアウト制約と相性が悪く、本格運用に耐えなかった。

**将来、常時稼働や定期実行を再導入したい場合の選択肢:**
1. 有料プラン ($7/月) で常時稼働
2. Render Cron Jobs を別 service として定義
3. `.github/workflows/full-pipeline.yml` に `on: schedule` を追加して GitHub Actions 上で
   cron 実行（Render 側を起こす必要なし）
4. 外部 cron-as-a-service (cron-job.org / UptimeRobot 等) で `/health` を ping
