...
| `[定常]` | 夜間スコア更新（`sector_ols`） | `nightly-scores.yml` | `nightly_scores.py`（Issue #432・親 #423）を実行し `sector_ols` を回して `regression_results`（`predicted_market_cap` / `gap_ratio`）を更新する。**起動は `daily-incremental` の `workflow_run` チェーンで `conclusion == 'success'` のときだけ**（株価が前進していない日にスコアだけ更新すると、古い株価由来の値が「今日のランキング」として出るため）。`regularization=ridge` 固定（既定 features 10項目は VIF>10 が頻発）。1モデルの失敗は他を巻き込まず、実行後に `max(computed_at)` を直接クエリして永続化を確認する（例外なし＝コミット済みとしない）。手動即時実行は `workflow_dispatch` | **16.1分**（2026-08-03 本番初回実走・run 30808053564・30業種/2,837社を更新。`timeout-minutes: 60`） |
