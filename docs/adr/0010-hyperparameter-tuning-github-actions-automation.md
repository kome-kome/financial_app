...
6. **GUI 手動トリガーを廃止し、読取専用バッジ＋自動プリフィルへ一本化する**
   ...
   **→ この「無条件の自動反映」は 2026-09-20（#711）で条件付きになった**
   （[ADR-0047 の追記](0047-tuning-gate-compares-on-one-panel.md)）。保存値を測ったパネルが
   現在と同じだと積極的に言えたときだけ反映する＝**パネルは毎晩伸びるので実際には探索当日
   しか働かない**。バッジと手動適用のボタンは残る。ボタンのラベルも #604 で
   「初期値にリセット」→「調整済みの値に戻す」へ改名済み（押すとチューナの選んだ値へ戻るので、
   「初期値」は嘘だった）。
