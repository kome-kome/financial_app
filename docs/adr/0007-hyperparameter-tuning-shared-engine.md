...
## Update（2026-07-11・Issue #304）

Issue #299 が当初スコープ外とした改善案のうち、**統計的妥当性に影響しない4案**（設計フェーズで
確定・CVフォールド数削減は明示的にスコープ外）を実装した。

1. **`winsorize`/`normalize`/`fit_feature_columns`（`plugins/utils.py`）の numpy ベクトル化**。
   要素ごとの変換（ソート・比較・クリップ・`log`・減算・除算）は numpy でベクトル化しつつ、
   平均・標準偏差の集計（`statistics.mean`/`stdev`・NaN補完平均の`sum()/len()`）は Pure Python
   のまま維持した。`np.mean`/`np.sum` は総和の演算順序が異なり丸め誤差が生じ得る
   （実測: ランダム入力での不一致率 mean 約57%・std 約23%・sum 約35%）ため、これらは
   ベクトル化せず、旧実装とビット単位で完全一致する数値を返すことを
   `tests/test_utils.py::TestWinsorizeNormalizeExactMatch`（旧実装をそのまま複製した参照実装
   との比較・ランダム入力200〜50ケース＋境界ケース：`n<4`・全同値・NaN混入・空配列・
   log の 0/負値）で検証した。
2. **BIC選択（`select_features_bic`）のコスト計測**。本番DB読取専用で計測した結果、
   M-1 フルパイプラインに占める割合は 15.82%（199.08秒中31.49秒）で、支配的ではないが
   無視できない水準だった。`LassoLarsIC` 自体のアルゴリズム変更は統計的妥当性に影響し得る
   ため本 Issue のスコープ外とし、計測結果を `select_features_bic` のdocstringに記録するに
   とどめた（対応要否の判断材料として残す）。
3. **M-3（`macro_dlm.py`）の `load_prices`/`load_macro_levels` キャッシュ化**。#298 の
   `tuning_snapshot_cache()`（`plugins/macro_snapshots.py`）を拡張し、`load_data`/
   `preload_macro`/`build_snapshots` 専用だった名前空間辞書に `load_prices`/
   `load_macro_levels`/`cv_by_selected_features` を追加。他モジュールから同じキャッシュ機構
   を使うための汎用アクセサ `tuning_cache_get_or_compute(namespace, key, compute)` を新設し、
   `plugins/macro_dlm.py` の該当2関数をこの経由でラップした（`_load_prices_impl`/
   `_load_macro_levels_impl` へ実体を分離する既存パターンを踏襲）。`plugins/tuning.py::search()`
   は既に `tuning_snapshot_cache()` で探索ループ全体を包んでいるため、`search()` 自体への
   変更は不要（docstring更新のみ）。M-3 は macro_features を探索軸に含まない
   （#267 設計方針）ため、294候補フルグリッドでも `load_prices`/`load_macro_levels` は
   探索全体で実質1回しか実行されなくなる。
4. **M-1（`macro_risk_return.py`）の BIC選択結果キャッシュ**。`walk_forward_cv_monthly()`
   呼び出しを同じ `tuning_cache_get_or_compute("cv_by_selected_features", key, compute)` で
   包み、キーを `(id(samples_by_ym), tuple(sorted(selected_names)))` とした。異なる
   `max_features` 候補間で BIC 選択結果（`selected_names`）が偶然一致した場合（実測でも
   `max_features=30`と`40`で完全一致を確認）、`samples_by_ym`（#298 でキャッシュ済み・
   構造パラメータが同一なら同一オブジェクト）が同じである限り `samples_sel` も数学的に
   完全に同一になるため、CV の再実行は真の重複計算（近似ではない）。

**設計判断（配置）**: 3./4. とも既存の `tuning_snapshot_cache()`（探索中のみ有効になる
`contextvars.ContextVar`）を再利用した。`macro_dlm.py`・`macro_risk_return.py` はいずれも
既に `macro_snapshots.py` に依存済み（`oof_backtest`・`get_producer_scores`・
`select_features_bic` 等）のため、汎用アクセサを追加してもモジュール間の依存方向は増えない。
「探索中はキャッシュを有効化する」という意味論を1つのコンテキストに集約する方が、
新しいコンテキストを乱立させるより一貫性がある。

**テスト**: `tests/test_tuning.py::TestTuningCacheGetOrCompute`（汎用ヘルパーのキー一致/
名前空間分離/コンテキスト外パススルー/未知名前空間の`KeyError`）、
`tests/test_macro_dlm.py::TestTuningSnapshotCacheForDlm`（`load_prices`/`load_macro_levels`
のキャッシュヒット/ミス・`search()`統合で呼び出し回数1回を確認）、
`tests/test_macro_risk_return.py::TestCvResultCacheBySelectedFeatures`（`selected_names`一致
時にCV呼び出しが1回に減ること・不一致時は2回のまま・コンテキスト外では常に2回であることを
確認）を追加。全件 `pytest`（1000件超）green。

**実測**（本番DB・読取専用・`persist=False`・Issue #298/#299 と同一手法）:

- **M-1 フルパイプライン（`max_features` 6値グリッド `[5, 10, 15, 20, 30, 40]`）**:
  Before（#299・本Update前）266.65秒 → After（本Update・1〜4適用後）**199.08秒**。
  **約1.34倍高速化（25.3%削減）**。各候補の `best_score`（0.2563・0.2498・0.2379・0.2326・
  0.2520・0.2520）は Before/After で完全一致（統計的結果への影響なしを実データでも確認）。
  `max_features=30`と`40`のスコアが完全一致しており、案4のキャッシュが実データでも
  ヒットする実例であることを確認した。
- **M-3（`n_iter=4`・random・seed=0）**: Before（#299 Update 記載値）215.54秒 → After
  （案3適用後）**160.48秒**。**約1.34倍高速化（25.5%削減）**。M-3 は `dlm_filter` 自体の
  呼び出しを省略できない構造（#299 Update参照）のため、`load_prices`/`load_macro_levels`
  のキャッシュ化のみが効いている。n_iter=4 のような小さいサブセットでは4候補全てが
  同一構造（macro_features固定）のため既に高い相対効果が出ている。

**残課題（288候補フルグリッドの見通し）**: Issue #299 と同じ粗い外挿手法
（6候補サブセット→288候補への単純比例）を適用すると、本Update後の288候補推定所要時間は
約340分（#299時点の見積り）÷1.34 ≈ **約254分**となり、`timeout-minutes=240` を
依然としてわずかに超過するリスクが残る。この外挿は「1構造パターンの測定を48通りへ単純
比例」させた粗い推定であることに加え、案4（BIC選択結果キャッシュ）の効き方は
`max_features` グリッドの並びとBIC選択結果の偶然の一致に依存するため、288候補フル実行では
本測定（6候補中2件がキャッシュヒット）より有利にも不利にも振れ得る不確実性が上乗せされる。
正確な検証には288候補フル実行または GitHub Actions 月次自動実行（`tune-hyperparameters.yml`）
の実績ログが必要。今回のスコープ外とした **CVフォールド数削減（`step_months` 変更）**は
統計的トレードオフ（フォールド数減＝OOF評価の粒度低下）を伴うため、対応要否をユーザー判断
に委ねる新Issueとして起票した（Issue #304 コメントに記載）。

## Update（2026-07-20・M-3 `auto_hyperparams` 撤去）

Considered Options 第4項で「却下」とした **M-3 の in-UI `auto_hyperparams`（周辺尤度）の扱いを
改め、機能ごと撤去した**（schema・execute 分岐・`_auto_select_hyperparams`・`_AUTO_SAMPLE_N`・
diagnostics `auto_hyperparams_used`）。当時の維持理由「高速な in-UI フォールバックの価値」は、
本 ADR 項目6の tuned 既定値の UI 自動注入＋ADR-0010 の月次自動チューニング運用が定着した現在、
実質的に消滅した（tuned 未適用の新環境でしか出番が無く、単一本番運用では発生しない）ため。

撤去の積極的理由は誤操作リスク：UI 既定値は常に OOF rank-IC 最適（tuned 注入）であり、
チェックボックスを有効にすると in-sample 周辺尤度で選んだ δ/β_v が**それを上書き**する。
「チェック＝より良い自動設定」という直感と効果が逆（目的関数不一致：1期先予測密度の当てはまり
≠ 横断ランキング性能）で、UI 文言では防ぎきれないと判断した。

`_AUTO_DELTA_GRID`/`_AUTO_BV_GRID` は `tuning_search_space()` の δ/β_v 探索空間として存続する
