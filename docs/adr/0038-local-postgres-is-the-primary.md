...
## 追補: 断面への書き込みを API で止める（2026-09-25・#733）

決定4の「閲覧用の断面」と追補の決定1（断面は凍結＝bloat が増えない）は、**運用上の決まり**で
しかなかった。Render で止めていたのは全件収集・株価履歴・J-Quants の3本だけで、差分収集・
市場データ更新・マクロ収集などは通り、断面へ書き込んでいた。断面だけが前進すると正本とも
凍結断面とも違う状態になるが、**どちらの DB でも書き込みは成功するのでエラーにならない**。

- 収集ルーター（`/api/collect/*`・`/api/scheduler/*`）の GET 以外を、ルーター単位の依存で一律
  403 にする。エンドポイントごとの `if` にしないのは、足したエンドポイントに掛け忘れても失敗と
  して現れないから。テストはルートを実体から列挙して全部叩く。停止系も止める（起動系が全部
  403 ならプロセス内に止める対象のジョブが無く、reset-stuck は `CollectionLog` を書き換える）。
- 条件は `RENDER_LIGHT_MODE` **または接続先が prod**（`api.writes_blocked()`）。旗だけにしないのは、
  決定1b と同じく render.yaml の env が Blueprint 管理外のサービスでは反映されないことがあるから。
  接続先 prod は「断面へ書かない」という決まりそのもので、ローカルから `FINAPP_DB_TARGET=prod`
  で起動した場合も止まる。
- heavy プラグインの 403（`/api/plugins/{name}/run`・model-comparison）も同じ条件へ揃える。
  HTTP 経由で DB へ書くプラグインは `sector_ols`（`regression_results`）だけで、これは heavy。
  旗だけで判定していると、旗が抜けた Render で断面へ書く穴が1本残る。代償として、ローカルから
  prod へ繋いだときは書き込まない heavy（M-1/M-2/M-3 等）も実行できないが、断面は古いので
  そこで回す意味は薄い。
- 収集ルーターの外に置く GET 以外のルートは `api.WRITE_ROUTE_EXEMPTIONS` へ「断面へ書かない根拠」を
  付けて登録し、テストが実体と照合する。ルーター依存は外には掛からないので、別のルーターへ
  書き込み系を足しても失敗として現れないため。
- **凍結するのはデータで、スキーマではない。** 起動時（`lifespan`）の書き込み3つ——`init_db()` の
  DDL・`running` のまま残った `CollectionLog` のリセット・`APP_PASSWORD` の平文→ハッシュ移行（#345）
  ——は止めない。`init_db()` は ADR-0048 の指紋が変わったデプロイの初回だけ DDL を出し、断面の
  スキーマをコードへ追随させる。止めると新しいコードが断面に無い表・列を読んで画面が壊れ、
  パスワード移行を止めると断面に平文が残る。「断面へ書いている」ように見えても塞がないこと。
...
- Issue #733（断面への書き込みを API で止める＝2つ目の追補）
