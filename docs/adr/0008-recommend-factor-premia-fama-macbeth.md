...
**追記（2026-08-24・Issue #529 / [ADR-0041](0041-preset-weight-gate-has-an-implementation.md)）:
この実測は使い捨てのスクリプトで行われ、リポジトリに残っていなかった。`scripts/preset_ic_gate.py`
として実体化し、上表を再現できることを確認した。**

```powershell
python -m scripts.preset_ic_gate --cache-panel --until 2025-07 --compare-standardization
python -m scripts.preset_ic_gate --cache-panel --until 2025-07 --preset バランス型 --premia-run-id rfp_20260820T183244Z --no-cross-section-standardize
```

| 項目 | 上表の記録 | 再現（61期・2026-08-24） |
|---|---|---|
| 成長重視 是正前 → 是正後 | +0.1264 → +0.0605 | +0.1263 → **+0.0605** |
| バランス型 | +0.0316 → +0.0210 | +0.0316 → **+0.0210** |
| 割安重視 | −0.0183 → −0.0107 | −0.0184 → **−0.0108** |
| 高収益重視 | −0.0134 → −0.0107 | −0.0135 → **−0.0108** |
| 統計的最適化 | −0.0807 → +0.0960 | −0.0808 → **+0.0960** |
| 成長重視の差 | −0.0658 CI[−0.0850,−0.0484] p=0.001 | **−0.0659 CI[−0.0850,−0.0485] p=0.0010** |
| 他3プリセットの差 | いずれも ns（p=0.478 / 0.156 / 0.585） | **ns（0.4758 / 0.1579 / 0.5847）** |

差は最大 0.0003（3日分のデータ世代差）。**手続きは再現された。** 併せて上表の読み方を2点補う:

1. **並ぶ p=0.001 はブートストラップの下限**（`bootstrap_mean_ci` の Davison-Hinkley フロア
   `2/(n_boot+1)`・既定 n_boot=2000）であって、それ以上小さい p を意味しない。
2. **Bonferroni を「6検定」と書いているが上表は5行**で、6つ目が何だったかを表から追えない。
   実体側は α と検定数を実際の検定回数から出して必ず印字する（ADR-0041 Decision 5）。

`gap_ratio` を持たないパネルの限界（上記(1)）も、実体側では**プリセットごとの「測れなかった
重み比率」**として表に出る（割安重視 44.4% / バランス型 9.8% / 成長重視 7.0%）。既定では
20% 超の行を判定から外すので、割安重視の rank-IC が結論に使われることはない。

**追記（2026-08-25・Issue #520）: 消費側の母集団が年度混在であることを実測し、「実害なし・
現行が正しい」で決着した。併せて Decision 1 の充足率を再測定した。**

`recommend.execute` / `backtest.run` は `latest_year_subq`（各社の最新年度を1行）で母集団を
作るため、`fit_view_metric_stats` は**異なる会計年度の値を1つの分布で標準化している**。
`gap_ratio` は年度窓の正規化を通らない生の％なので、ここが歪みの入口ではないかという疑いが
#520 だった。ローカル正本（3,724社）で測った結果:

| 測ったこと | 実測 |
|---|---|
| 年度混在の規模 | 2025年度 1,249社 / 2026年度 2,475社 ＝ **33.5% : 66.5%**（「数社」ではない） |
| `year` の正体 | **決算期の違い**。year=2025 は 2025-12 期(537) / 2025-09 期(189) / 2025-06 期(157)、year=2026 はほぼ 2026-03 期(2,141)。直近決算からの経過はどちらも 5〜8ヶ月で**鮮度は同程度** |
| `gap_ratio` の年度別 (mean, sd)（winsorize 後） | 混在 (+46.05, 120.12) / 2025 (+54.23, 157.34) / 2026 (+42.72, 105.27) |
| その差はモデル差か | **違う**。両年度を持つ 1,723社で median diff (2026−2025) = **+1.11**、2026 が大きい会社 **51.4%** ＝ **系統的シフト無し**。`regression_results` も両年度とも 2026-08-24 の夜間バッチで更新済み |
| 年度別標準化にした場合の順位 | Spearman **0.9079** / top100 重複 **76%** / top100 の 2025年度組が **43 → 26社** |

**判断: 年度別に標準化し直さない。** `year` が決算期の違いである以上、年度で切ることは
12月期決算と3月期決算を別々の分布で評価することを意味し、「同じ市場で割安な銘柄を探す」目的に
反する。順位は動くが、同一企業の年度間に系統的シフトが無い以上**動いた先が正しい根拠は無い**。

**#520 が置いた前提2つは実測で否定された:**

1. 「VIEW の `z_*` は年度窓で正規化済みだから混ざっても壊れない、`gap_ratio` だけが危ない」
   → **非対称は成り立たない**。`z_op_margin` の sd は年度で **53倍**違う（2025年度 0.0159 /
   2026年度 0.0003）。上の #469 の機構（共通分母 `pl_revenue` の外れ値が `STDDEV_SAMP` を
   支配する）がそのまま出ているだけで、**#509 の断面標準化はまさにこの歪みを取り除いている**。
   `gap_ratio` を同じ扱いに含めた判断（上の #509 追記・決めたこと1点目）は正しかった。
2. 「本番 `execute` と `backtest` で母集団の定義が違うかもしれない」 → **同じ**。
   `backtest.run` も `max(year)` の年度混在で、`period_end <= start_date` の as-of フィルタが
   付くだけ。#509 の検証が本番と別の母集団を測っていた、という懸念は当たらない。

**Decision 1 の充足率の再測定（2026-08-25）**: 2024年度以前 **0%**（2024 は1社のみ）/
2025年度 **75.4%**（2,951/3,916）/ 2026年度 **94.7%**（2,408/2,544）。直近2年は当時（67% /
72%）より改善しているが、**2024年度以前が 0% であることは変わらない**。`build_snapshots` は
全指標が同時に非NULL の企業だけを採るため、2020-07 起点のパネルに `gap_ratio` を載せると
有効期間がほぼ消える構造は不変＝**Decision 1 の除外は維持**（ADR-0041 が「#520 で判断」と
保留していた件も、これで「載せない」で確定）。

再現コマンドは Issue #520 のコメントに残してある（DB を直接集計する ad-hoc クエリで、
...
1. **母集団・目的変数・fold は M-1/M-2/M-3 と共有する。ただし gap_ratio は回帰の
   ...
   **gap_ratio を含めなかった理由**: 本番DBを直接集計したところ、`gap_ratio`
   ...
   過去年度へ遡及していないため。**充足率は 2026-08-25 に再測定して更新されているが、
   2024年度以前が 0% であることは変わらず本除外の結論は維持される**——末尾の #520 追記）。`build_snapshots` の `fin_features` は全指標が同時に
