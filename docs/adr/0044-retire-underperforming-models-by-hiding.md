...
- 残る非対称: M-2 は M-6 に有意劣後（p=0.002）しているが、M-4 の基底・SHAP 解釈・
  ...
  M-4 を退役させた以上「M-2 は何のために残っているか」は別途明文化する（**#572**）。
  M-4 の基底という支えが外れ、R1'（ADR-0020）は M-6 も持つため、M-2 固有の価値は
  実質「SHAP による非線形の解釈」と「比較の基準線」の2つに絞られている。
  **→ [ADR-0052](0052-m2-stays-for-interpretation-not-supply.md) で決着**（2026-09-07）。
  M-2 は `hidden` にせず `mu_source` の選択肢からだけ外す＝本 ADR の2状態に対する
  **第3の状態「表示するが供給者ではない」**を置いた。
