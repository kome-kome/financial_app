...
（EPU チャネル＋昇格ゲート手順）の続編。**2026-07-31 追記: Issue #409 で週次 M-3（DLM）でも
再判定し、こちらも非有意（「追記」節）**。
...
- **ニュースのトーン・報道量・Wikipedia 関心度は、既存マクロ71本の上に情報を足さない**（※**「71本」はマクロ系列の本数ではない**——上の「71特徴量」は `macro_feature_bakeoff` の `n_features`＝**M-2 既定 config で組んだパネルの全特徴量列数**で、内訳は財務25＋マクロ46。測定コミット `caac598`（2026-07-31）を数え直した実測で、当時のマクロは `MACRO_FEATURE_OPTIONS` 53・`DEFAULT_MACRO_FEATURES` 46 だった。**実測の結論は変わらず、誤っていたのは数につけたラベルだけ**・#717）。5系列を
...
## 追記: 週次 M-3（DLM）での再判定（2026-07-31・Issue #409）

上の「日次シグナルを活かすなら週次の M-3 側が筋」は**仮説のまま**残っていた（月次スナップ
ショットは月末の値を1点だけ使うため、日次スパイクがほぼ落ちる＝月次での棄却は週次での棄却を
意味しない）。この5系列は ADR-0012 の M-3 適格条件（週次高頻度ファクター）を満たす唯一の
未消化ストックだったため、追加収集ゼロで再判定した。

**M-3 側の登録**（`plugins/macro_dlm.py::_DLM_MACRO_MAP`）:

| ファクター | series_code | 変換 |
|---|---|---|
| `dlm_news_tone` | `JP_NEWS_TONE` | **週次差分**（正負を跨ぐため log 比が定義できない） |
| `dlm_news_econ_tone` | `JP_NEWS_ECON_TONE` | **週次差分**（同上） |
| `dlm_news_econ_vol` | `JP_NEWS_ECON_VOL` | 週次対数リターン（報道量%＝常に正） |
| `dlm_wiki_market_attn` | `JP_WIKI_MARKET_ATTN` | 週次対数リターン（閲覧数＝常に正） |
| `dlm_wiki_macro_attn` | `JP_WIKI_MACRO_ATTN` | 週次対数リターン（同上） |

`_DLM_MACRO_MAP` は当初から `logret`/`diff` の2軸を持つ（金利が `diff`）ため、トーンの差分
変換に `_build_series` の分岐追加は不要だった（Issue の改善案2は起票時点の推測で、実装時に
不要と確認）。`JP_NEWS_ECON_VOL` は GDELT の欠測日に 0 が入りうる（実測 2025-12-06 の1日）が、
非正水準は既存の logret ガードが週ごと除外する。現在の週次グリッド（2021-01-04〜2026-07-13・
289週）には非正水準が乗らず、OOF サンプル数は base と完全一致した。

**実測**（`scripts/macro_dlm_feature_bakeoff.py --preset attention`・3,979社 / 49期 /
652,247 OOF ペア。base = 現行 `DEFAULT_MACRO_FEATURES` 22ファクター、with_cand = base + 本節の
5ファクター＝27）:

| 指標 | base | with_cand | 差 | 95%CI | p | 判定 |
|---|---|---|---|---|---|---|
| 買い側 rank-IC | +0.0123 | +0.0122 | **−0.0001** | [−0.0010, +0.0007] | 0.807 | ns |
| 売り側 short_side_spread | +0.00131 | +0.00126 | −0.00005 | [−0.00019, +0.00008] | 0.513 | ns |

**VERDICT: keep as option only**（1モデル × 2指標 = 2検定・Bonferroni α=0.025 で通過ゼロ）。
コストは状態次元 23→28 で実行時間 69.3s→75.5s（+9%）、OOF サンプル数は 652,247 で不変。
5系列は M-3 側でも `_GATE_REJECTED_FEATURES`（実測済み・棄却）へ置く。

確定知見（更新）:

- **日次のニューストーン／関心度は、月次でも週次でも既存マクロの上に情報を足さない**。週次
  （M-3・同時点ファクタ応答）に持ち込んでも rank-IC は −0.0001（p=0.807）で、月次で見えなかった
  日次スパイクの寄与は**存在しない**（「スナップショット頻度が落としていた」という仮説の否定）。
  以降、GDELT / Wikimedia 系の再提案は本節と上の月次実測の**両方**を覆す根拠（別の集約軸＝
  銘柄別・業種別など、または別のモデル族）を伴わない限り採らない。収集自体は継続するため、
  将来レジームが変わったら `--features` で再判定できる。
- **昇格ゲートはモデル・頻度ごとに独立に持つ**。M-3 にも `_PENDING_EVAL_FEATURES` /
  `_GATE_REJECTED_FEATURES` を新設し、`DEFAULT_MACRO_FEATURES = 全選択肢 − 両枠` へ変更した
  （従来は「全選択肢＝既定」で、選択肢に足すと無条件に既定 ON になり状態次元の増加を全ユーザーが
  無検証で負っていた）。検定数はモデル数 × 指標数で決まるため、M-3 単独では α=0.025。
