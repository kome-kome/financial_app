...
対処: `summarize_diagnostics` を `round_to="none"`（生値）へ修正し、丸め回帰を検知する単体テスト（`TestSummarizeDiagnostics`）を追加。`scripts/experiment_pooled_rhat.py` は本関数を再利用するため、再実行すれば 4桁精度で傾向を測り直せる。**#341/#356 の「床」に関する判断は、生値での再実測が出るまで保留**とし、それまで cron の閾値 1.05 は現状維持（緩和側なので誤 persist リスクは増えない）。教訓は [GOTCHAS.md](../GOTCHAS.md#分析計算の実装メモ)「閾値比較に使う統計量は表示用整形を経ていない生値であることを確認する」へ一般形で記録した。
