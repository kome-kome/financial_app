...
> **追記（2026-09-26・[#739](https://github.com/kome-kome/financial_app/issues/739)）**: 上の1か所で保留の窓を効かせた。
> `compute_ledger` が窓に入る公式イベントを外してから検出器と `Ledger.official` へ渡し、外した一覧を
> `stats["withheld_official"]` に残して毎晩ログへ出す。外したイベントは検出器から見て「無かった」ことになるので、
