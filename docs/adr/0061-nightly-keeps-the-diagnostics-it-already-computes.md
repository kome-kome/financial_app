...
- 残す価値の根拠は既にあった。[ADR-0058](0058-ridge-alpha-is-chosen-by-loo.md) の実測では、33業種のうち
  ...
  （下端については、実際に候補の外まで振ると別の読み方になった＝Consequences の実測）
...
3. **読み手は読み出し CLI（`python -m scripts.nightly_diag_report`）だけ。自動起票はしない。**
   ridge α が候補の**上端**に張り付いた夜と、macro_enet の最終モデルの α がパスの端だった夜は夜間ログに
   WARN を出す。**ridge の下端は WARN にせず INFO に留める**（罰なしと同等で対処できない・実測は
   Consequences）。警報の基準は、実測が溜まってから別の Issue で
...
- 初回の記録で既に端が見えた（2026-09-23・ローカル）。sector_ols は ridge α が下端 0.001 の業種が3つ
  ...
  （17 fold 中2）が α パスの下端だった。OOF rank-IC は 0.1541
- **ridge の下端は「候補が狭い」ではなく「罰が要らない」だった**（2026-09-23・同じ夜の入力で α を
  0〜1e5 まで振って一個抜き誤差を測った）。特徴量は z-score 済みで X'X の対角は社数ほどあるので、
  0.001 の罰は α=0（普通の回帰）と区別が付かない。機械（208社）は α=0 でも 0.001 でも 0.5947 で、
  予測値の差は最大 7e-4（標準化単位）。下端を選んだ残り2業種（海運業 9社・鉱業 5社）は社数が係数の数
  （切片込み 11）より少なく、LOO 自体が当てにならないうえ、15社未満なので全社プールへの縮約が掛かる。
  上端（1000）を選んだ業種は0で、1000・1e4 はどの業種でも誤差が悪化した。**よって `RIDGE_ALPHAS` は
  広げない**（ADR-0058 決定1 のまま）。macro_enet の端（l1_ratio 0.1・fold の α パス下端）は未実測で、
  同じ読み方ができるとは限らない（#728）
