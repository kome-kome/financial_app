...
### 追試（2026-08-01・Issue #411）: strict はもう律速していない

本 ADR は Consequences で「次の律速はコモディティ8系列（2020-07 開始）まで緩む」と**推測**を書いた。
その推測が今も正しいかを `scripts/measure_strict_binding.py`（本追試で新設・読取専用・Egress ゼロ）で
実測した結果、**推測は現況と合わず、strict 制約は現在まったく律速していない**と確定した。

> **後日談（2026-08-01・ADR-0025）**: 下表の「窓はデータ履歴長で決まる」という結論を受けて
> 株価・財務を実際に過去へ延伸した。窓は 47→**71ヶ月**・fold 10→18 期になり、M-1 strict が
> 律速でない点は延伸後も変わらない（strict = nan_ok で同一母集団）。数値の最新版は ADR-0025 を見ること。

| 条件 | 月数 | レンジ | サンプル |
|---|---|---|---|
| strict（`macro_nan_ok=False`・M-1 既定・交差項ON） | 47 | 2021-08〜2025-06 | 111,210 |
| nan_ok（M-2 方針・他は M-1 契約のまま） | 47 | 同 | 111,210 |
| マクロ無し（M-1 契約） | 47 | 同 | 111,210 |
| 参考: M-2 の実契約（交差項なし・価格特徴あり） | 43 | 2021-12〜2025-06 | 57,955 |

- 既定マクロ **46本すべてが 2021-01（＝週次株価の開始月）から非 None**。`NEVER` はゼロ。
  コモディティ8系列は本 ADR 以降の再収集で本番 `macro_data` 上 **2016-07-25 開始**まで遡っており、
  「次の律速」と書いた前提自体が解消済みだった（truncate が続くのは HY_OAS/IG_OAS のみ・2023-06）。
- 3条件でサンプル数が **完全一致** ＝「選択中の全マクロが同時に非None」の行しか使わない strict の
  制約が、1行も落としていない。
- 実際の窓は **データ履歴長**で決まる: `stock_price_weekly`（2021-01-04〜・289週 → 52週先ラベル付き
  スナップショットの上限 53ヶ月）と `financial_records`（period_end 2020-09-30〜・先頭6ヶ月を削る）。

**帰結（本 ADR の Decision は不変・運用知見の追加）**:

1. **マクロ既定の増減や strict の設計変更では学習窓は伸びない**。窓を伸ばす議論は株価/財務の履歴
   延伸（＝ DB 容量トレードオフ）の話であり、モデル側の話ではない（Issue #411 で分離して管理する）。
2. **M-4 の共通 (ym,ec) 域を狭めているのも M-1 側ではない**。同実測の参考条件（表4行目）が示す通り
   M-2 の実契約が 43ヶ月 / 57,955 で最も狭く、M-1 build（47ヶ月 / 111,210）より小さい。
   ADR-0015 の「共通域は M-1(strict) 律速」という記述は誤りだったため同 ADR §実測へ訂正を追記した。
3. 「次の律速はどれか」を **prose の推測で残さない**。判定は `python -m scripts.measure_strict_binding`
   が `VERDICT` 行で毎回出す（`strict IS binding` なら latest-start 特徴を律速候補として列挙し、
   本 ADR と同じ手順＝既定から外す＋非truncate代替を入れる、へ誘導する）。
   新規マクロ系列の昇格ゲート（ADR-0023 / `scripts/macro_feature_bakeoff.py`）が strict 母集団の
   縮みを見るのと同じ役割を、既定集合そのものに対して継続的に果たす。
