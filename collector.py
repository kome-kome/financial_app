...
# バッチ処理のしきい値（DBコミット間隔・進捗通知・ポーズ）。値は据え置きの純粋な定数抽出。
COMMIT_BATCH         = 200   # 株価/市場データ更新の DB コミット間隔（件）
COMMIT_BATCH_HEAVY   = 50    # 1件あたりの処理が重い収集（全件収集・市場データ）の DB コミット間隔（件）
COMMIT_BATCH_REPARSE = 100   # XBRL 再解析 / CF 補完の DB コミット間隔（件）
MASTER_BATCH         = 200   # 企業マスタ保存の DB コミット＋expire 間隔（件）
PROGRESS_BATCH       = 500   # 進捗コールバック通知の間隔（件）
PROGRESS_BATCH_DENSE = 200   # 進捗コールバック通知の間隔（密め・件）
LOG_BATCH            = 100   # INFO ログ出力の間隔（件）
PAUSE_BATCH          = 100   # レート制限の追加ポーズ（BATCH_PAUSE）を挟む間隔（件）
...
async def backfill_historical_stock_prices_yahoo(
    ...
    async with httpx.AsyncClient() as session:
        for i, (sec_code, recs) in enumerate(sorted(by_company.items()), 1):
            ...
            if on_progress and (i % PROGRESS_BATCH_DENSE == 0 or i == total):
...
async def fill_recent_stock_price_gap_yahoo(
    ...
    async def _yahoo_batch_gen(session):
        for i, (sec_code, edinet_code) in enumerate(companies, 1):
            ...
            if on_progress and i % PROGRESS_BATCH == 0:
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        for i, rec in enumerate(targets, 1):
            ...
            try:
                ...
                # 一定件数ごとに commit（長時間トランザクション対策）
                if updated % COMMIT_BATCH_REPARSE == 0 and updated > 0:
                    db.commit()
...
async def update_market_data(db,
    ...
    async with httpx.AsyncClient() as client:
        ...
        for coro in asyncio.as_completed(tasks):
            ...
            _apply_price_to_record(latest, price)
            updated += 1
            if updated % COMMIT_BATCH_HEAVY == 0:
                db.commit()
...
async def _phase_upsert_master(db, client, on_progress, max_companies) -> tuple:
    ...
    # に切り替わるため、MASTER_BATCH 件ごとに commit + expire_all してトランザクションを短く保つ
    for i, (_, row) in enumerate(companies_df.iterrows()):
        ...
        if (i + 1) % MASTER_BATCH == 0:
            db.commit()
            db.expire_all()
        if on_progress and (i + 1) % PROGRESS_BATCH == 0:
...
async def _phase_process_docs(db, client, all_docs: list,
    ...
    for i, doc in enumerate(all_docs):
        ...
        try:
            ...
            if (i + 1) % COMMIT_BATCH_HEAVY == 0:
                db.commit()
                log.info(f"DB commit ({COMMIT_BATCH_HEAVY}件)")
            if (i + 1) % PAUSE_BATCH == 0:
