...
- **J-Quants catchup は `AdjC != C` の行を書かない**（#620・ADR-0053）。`close` は Yahoo と共有する列で、調整の中身が違う値を混ぜると帯の両端に**企業イベントではない段差**ができる——どちらも妥当な株価なので**エラーは出ない**。判定は返ってきた行の中で完結させる（DB の既存値は前夜までの上書きで汚れており自己参照になる）／丸め許容は `collector_utils.rounding_tolerance` を修復側と共有する／`C` 欠落は `scale_unknown` として別に数える。既存の帯は `scripts/repair_scale_mixture.py` が直すが、**仕組みを直す前にデータだけ直すと次の晩に書き戻される**。往復段差の検知は**形だけでは本物を選べない**（実測 候補234社・本物2社）ので、`AdjC != C` を報告した社と交差させる。**確定条件は3つで、Yahoo の現値との食い違いを必ず含める**——`AdjC != C` は「分割がある」としか言っておらず、Yahoo が正しく調整していれば DB 値も公式値も一致して誤検知になる（実測 E01717）。詳細は [GOTCHAS.md](docs/GOTCHAS.md)
