...
- **正本はローカル PostgreSQL**（#503・ADR-0038）。Supabase は**閲覧用断面（Render 専用）＋ Storage のバックアップ置き場**で、**Postgres へ書き戻す経路は作らない**（ADR-0035 の dest ローカル限定ガードはそのまま生きる／戻しても NANO の実効メモリに DB が乗らず必ず再発する・#500）。**重い計算は平日日中バッチ（`scripts/run_daytime.py`）のキューへ積み、走っている間は何も並走させない**——裏で作業すると同じ入力から違う答えが出る（発散が 0→344 に増えた実測あり）。**H1・会社予想の収集は日中枠の暦が毎月積む（手で積まない）／月次系の起動日と祝日・年末年始は並走に敏感な仕事を取り出さない**（#681・#684・ADR-0056。祝日表 `run_daytime.HOLIDAYS` は毎年足す）。**日中キューは「結論を出した失敗」を戻さないが、結論を出す前にプロセスごと消えた中断だけ in-flight マーカーで見分けて1回だけ先頭へ戻す**（#639）。**M-1 探索は窓に入らないので専用タスクのまま**で、`macro_beta_loadings` は探索の入力だから**月次系の起動日の前後は依存順そのもの**（#579・#584・ADR-0046）。**収束ゲートは変数ごとの `r_hat` の p99 で判定する（全パラメータの max ではない）**（#609）。**落ちた run は `status=quarantined` で保全し exit は非0**。**「走らなかったこと」の検知は `scripts/check_batch_freshness.py`**（#515・ADR-0042）——GHA はローカル DB を見られず、この役だけは原理的にあちら側へ置けない。**収集の入口は `_pipeline_incremental.py`**——`collector.py --incremental` は株価を1バイトも更新しない（取り違えてもエラーは出ない）。**バックアップは `scripts/run_backup.py` が回す**（手動 CLI へ戻さない＝取り忘れは失敗として現れない）／**`backup_push --dest storage` でもローカル世代の掃除は dest によらず走らせる**。起動日・時刻・窓・所要の実測は [DEPLOYMENT.md](docs/DEPLOYMENT.md) のバッチ暦、各バッチの仕組みと経緯は [ARCHITECTURE.md](docs/ARCHITECTURE.md) が正本。
...
- **バリュエーション基準の不一致は `financial_metrics` VIEW で補正し、`financial_records` の生値は書き換えない**（#655・ADR-0055）。**F は行の固有値ではない**＝その年より後のイベントの累積積なので、焼き付けると必ず陳腐化する（だから VIEW・だから毎晩全置換）。向きの唯一の源は `measure_split_valuation_bias.COLUMN_DIRECTION`／検出アルゴリズムは同モジュールの純関数を import して写さない／**検出0件なら既存の表に触らず失敗する**（消してから失敗すると補正が静かに全部外れる）／**`stock_price` は補正しない**ので補正行では `per <> stock_price / pl_eps`／**測定器は回帰テストにならない**（生値を読む）／**`PREPROCESS_VERSION` は上げない**。倍率の取り方は3経路とも条件付き: **第2経路は `bs_bps` の比ではなく翌年の `issued_shares` 比から取る／第3信号が無い年は採らない**（#659・決定4-3）、**翌年の行が無い最新年だけは公式 `AdjFactor`（`jquants_adj_factor_events`）で取る——rebuild から J-Quants を叩かない／表に行が無ければ採らない／CLI の突合には公式を渡さない**（#661・決定4-5）、**第1経路を公式の不在で外すのは受け取ったバーの区間（`jquants_adj_factor_coverage`）がイベント窓を覆うときだけ**（#668・決定4-8）。**純資産比チェックの許容（`DEFAULT_EQUITY_TOL`）は `verify-sample --census` と窓外の Yahoo split 照合をやり直さずに動かさない**（#657・決定4-4）。症状・実測値・再現条件は [GOTCHAS.md](docs/GOTCHAS.md)、決定の全文は [ADR-0055](docs/adr/0055-valuation-basis-mismatch-is-corrected-in-the-view.md)。
...
**導入後の脆弱性は `dependency-audit.yml` が週次で照合し、検出は notify-failure が起票する**（#723）。修正版へ上げる／外すなら `ignores` 配列に理由と Issue 番号を同じ行へ（黙って外さない）。
