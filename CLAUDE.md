...
| [batch-ops](.claude/skills/batch-ops/SKILL.md) | 起動・収集・各バッチ・株価修復の全コマンド | バッチ／収集／修復を実行するとき |
...
起動・収集・各バッチ・株価修復の全コマンドは [.claude/skills/batch-ops/SKILL.md](.claude/skills/batch-ops/SKILL.md)（スキル `batch-ops`）にある。
バッチを叩く／収集を回す／株価を修復する場面ではこのスキルを読むこと。テストは `pytest`（`pytest.ini` で `testpaths=tests` 固定）。
...
| `scripts/batch_common.py` | ローカル駆動バッチ（日次／月次／M-1）の共通骨格＝**「走らなかったことを検知する」仕組みの唯一の源**（足跡・通知・heartbeat・ステップ予算）。**子の出力はログへ直結する／生の接続文字列は出さない／Σ予算＋マージン ≤ 窓／予算は締切（`FINAPP_STEP_DEADLINE_UTC`）として子へ渡し、予算の無いステップでは消す**（ADR-0054。打ち切りは猶予ゼロなので、途中経過を残せるのは子が自分で畳んだときだけ）。詳細は [ARCHITECTURE.md](docs/ARCHITECTURE.md) |
...
| `scripts/check_batch_freshness.py` | **バッチ鮮度 watchdog**（ADR-0042）。起票・CLI・ログを持ち、判定は `batch_freshness.py` と共有する。**閾値は `cadence + 窓` の導出（実測から逆算しない）／バッチの判定は `*_last_run` のみ／成果物の固着は `PRODUCERS` で別に起票する（バッチ失敗の Issue を閉じても穴は残る・#504）／通知経路（`gh`）は健全な回にも毎回確かめる／ok へ戻った起票は自動で閉じる（手で閉じなくてよい。コメントを付けると自動クローズは止まる・#635）**。exit 0/2/3。詳細は [ARCHITECTURE.md](docs/ARCHITECTURE.md) |
...
- **正本はローカル PostgreSQL**（#503・ADR-0038）。Supabase は **2026-08-07 の閲覧用断面（Render 専用）＋ Storage のバックアップ置き場**。2回目の停止の真因は Egress ではなく **NANO の実効メモリ 408MB に DB が乗らないこと**（#500）で、VACUUM FULL 後も余裕は 13〜28MB＝週次株価が増え続ける以上、正本を置く限り必ず再発する。**Supabase の Postgres へ書き戻す経路は作らない**（だから ADR-0035 の dest ローカル限定ガードはそのまま生きる）。収集・スコア更新は `scripts/run_nightly.py`（タスクスケジューラ・JST 17:20）、月次は `scripts/run_monthly.py`（毎月1日 JST 01:00・#504）が Fama-MacBeth 重みと **M-2/M-3 探索**を回し、**M-1 マクロβ推論は `scripts/run_monthly_beta.py`（毎月2日 JST 01:00・#579）・M-1 探索は `scripts/run_monthly_m1.py`（毎月3日 JST 01:00・#584・ADR-0046）が別タスクで回す**——前者は実測 360分・後者は約752分でいずれも本体の窓（960分）に入らず、月次に置くと毎月ごく一部しか探索できない。**重い計算は平日日中バッチ（`scripts/run_daytime.py`・平日 JST 08:00・#618）のキューへ積み、走っている間は何も並走させない**——2026-09-07 に同一パネル・同一設定・同一コードで発散が 0→344 回に増え隔離された実測があり、本番の推論経路にスレッド固定が無い以上「裏で作業しない」ことが再現性の条件そのものになる。**M-1 探索（752分）は窓8hに入らないので専用タスクのまま**。**日中キューは「結論を出した失敗」を戻さないが、「結論を出す前にプロセスごと消えた」中断は in-flight マーカーで見分けて1回だけ先頭へ戻す（2回目は起票して捨てる）**（#639）。**H1・会社予想の収集は日中枠の暦が毎月積む（手で積まない）／月次系の起動日（1〜3日）と祝日・年末年始は日中枠が並走に敏感な仕事を取り出さない**（#681・#684・ADR-0056。祝日表 `run_daytime.HOLIDAYS` は毎年足す）——2026-09-09 に Windows Update の再起動が M-3 探索を 285分・255/294件で殺し、計算とキューの1件が同時に消えたが、足跡は閾値の内側で watchdog も鳴らなかった。**この2本の起動日の前後は依存順そのもの**（`macro_beta_loadings` は M-1 探索の入力なので、探索が先だと常に前月の値を見る）。**収束ゲートは変数ごとの `r_hat` の p99 で判定する（全パラメータの max ではない）**（#609）——max 順序統計は銘柄が増えれば必ず閉じ、実測で5週間 M-1 の入力が固着した。**落ちた run は `status=quarantined` で保全し exit は非0**——捨てると6時間が消え、静かに成功扱いにすると固着に気づけない、**「走らなかったこと」の検知は `scripts/check_batch_freshness.py`（毎日 JST 20:00・#515・ADR-0042）が担う**——GHA はクラウドなのでローカル DB を見られず、この役だけは原理的にあちら側へ置けない。**収集の入口は `_pipeline_incremental.py`**——`collector.py --incremental` は `run_full_collection` だけで**株価を1バイトも更新しない**（取り違えてもエラーは出ない）。**バックアップは `scripts/run_backup.py`（毎週日曜 JST 21:00・#606）が回す**——手動 CLI のままだと実効 RPO が「最後に人が思い出した日」になり、取り忘れは失敗として現れない。復元は `scripts/backup_restore.py`（Storage は 50MB/ファイル・1GB。表ごと分割で実測 38.1MB/世代）。**`backup_push --dest storage` はローカルへダンプしてから上げるので、ローカル世代の掃除は dest によらず走らせる**（storage 側だけにかけると週 37.5MB が正本と同じディスクへ積み上がる）。
- **ミラー（`scripts/mirror_*.py`）の書き込み先はローカル限定**（#481・ADR-0035）。`guard_dest_local()` がリモート dest を `SystemExit` で弾く＝**ローカルから本番 DB へ書く経路をコードとして持たない**（#503 の反転後もこの向きは変わらない）。エンドポイント解決は `database.resolve_database_url()` へ委譲し**二重実装しない**。restore は **FK 依存順に1表ずつ**（ダンプの TOC はアルファベット順であって `--table` の指定順ではない）。週次の再取得窓は `DAILY_WINDOW_DAYS` から導出（27週）。ミラー範囲は `Base.metadata.sorted_tables` から `xbrl_raw_documents`（0行）を除いた全表（表を足したら `SYNC_PLAN` へ1行足す＝`test_mirror_common.py` が照合する）＝`stock_price_daily` は #503 で範囲へ入れた（正本が移れば daily はローカルにしか無い正本データになる）。
...
- **J-Quants catchup は `AdjC != C` の行を書かない**（#620・ADR-0053）。**登録済みスピンオフ（`SPINOFF_ADJUSTMENTS`）の権利落ち前の日付も書かない——係数を掛けて書く経路を作らない**（#651）。`close` は Yahoo と共有する列で、調整の中身が違う値を混ぜると帯の両端に**企業イベントではない段差**ができる——どちらも妥当な株価なので**エラーは出ない**。判定は返ってきた行の中で完結させる（DB の既存値は前夜までの上書きで汚れており自己参照になる）／丸め許容は `collector_utils.rounding_tolerance` を修復側と共有する／`C` 欠落は `scale_unknown` として別に数える。既存の帯は `scripts/repair_scale_mixture.py` が直すが、**仕組みを直す前にデータだけ直すと次の晩に書き戻される**。往復段差の検知は**形だけでは本物を選べない**（実測 候補234社・本物2社）ので、`AdjC != C` を報告した社と交差させる。**確定条件は3つで、Yahoo の現値との食い違いを必ず含める**——`AdjC != C` は「分割がある」としか言っておらず、Yahoo が正しく調整していれば DB 値も公式値も一致して誤検知になる（実測 E01717）。詳細は [GOTCHAS.md](docs/GOTCHAS.md)
- **バリュエーション基準の不一致は `financial_metrics` VIEW で補正し、`financial_records` の生値は書き換えない**（#655・ADR-0055）。株価は遡及調整済みだが分母の1株指標は提出当時の基準なので、分割より前の年の行だけ per/pbr/market_cap が小さく div_yield/nc_ratio が大きい（実測 1,948行/451社・F の p50=2.00）。**F は行の固有値ではない**＝その年より後のイベントの累積積なので、焼き付けると必ず陳腐化する（だから VIEW・だから毎晩全置換）。向きの唯一の源は `measure_split_valuation_bias.COLUMN_DIRECTION`／検出アルゴリズムは同モジュールの純関数を import して写さない／**検出0件なら既存の表に触らず失敗する**（消してから失敗すると補正が静かに全部外れる）／**`stock_price` は補正しない**ので補正行では `per <> stock_price / pl_eps`／**測定器は回帰テストにならない**（生値を読むので補正後も同じ行数を報告する）。**`PREPROCESS_VERSION` は上げない**＝`build_period_panel` は z_* 7列だけで per/pbr を含まない。**第2経路（株数が追随しない社）の倍率は `bs_bps` の比ではなく翌年の `issued_shares` 比から取る／第3信号が無い年は採らない**（#659・ADR-0055 決定4-3。bps は内部留保でも増えるので比は `F/(1+g)` になり、倍率だけ系統的に1段小さく出る＝一致率 0.367。どちらも妥当な倍率なのでエラーは出ない）。**翌年の行が無い最新年だけは catchup が残した公式 `AdjFactor`（`jquants_adj_factor_events`）で倍率を取る——rebuild から J-Quants を叩かない／表に行が無ければ採らない／CLI の突合には公式を渡さない**（#661・決定4-5）。**第1経路を公式の不在で外すのは、社単位で受け取ったバーの区間（`jquants_adj_factor_coverage`）がイベント窓を覆うときだけ——区間は要求した期間でなく返ったバーで書き、0 本の社は書かない**（#668・決定4-8。J-Quants は扱わない社を 0 本で返し「イベントが無い」と同じ形になる）。**第1経路の純資産比チェックの許容（`DEFAULT_EQUITY_TOL`）は `verify-sample --census` と窓外の Yahoo split 照合をやり直さずに動かさない**（#657・決定4-4。余裕は 0.015 しかなく、下げると本物の分割を落とす。偽陽性率は抽出では測れない）。詳細は [GOTCHAS.md](docs/GOTCHAS.md)
- **TTM 行は `financial_metrics` へ混ぜない。読むのは `plugins.macro_snapshots.use_fin_rows("with_ttm")` の内側だけで、そこでは本番の表へ書かない**（#424 子2・ADR-0051）。既定は通期のみで、昇格ゲート（子3）は未実施。切替の内側は `tuning_dry_run` を強制し `recommend_factor_premia.persist` は拒否する（永続化行は行の基準を持たないので、書くと本番が「TTM で推定した重み × 通期の断面」を読む＝#509 と同型で `PREPROCESS_VERSION` では検出できない）。`shared_snapshot_cache()` の内側で基準を変えない（断面キャッシュ上限1・M-1 の CV キャッシュは `id()` をキーにしており、前の基準の結果が返りうる）。**材料（前期通期・前期H1・今期H1）の間に分割がある社・年度は合成しない**——株数比で1株指標を直すと消却・増資まで分割として扱う。**弾いた理由と引き継いだ列は毎晩数えて出す**（ゼロが続くのは判定が効いていない合図）。詳細は [GOTCHAS.md](docs/GOTCHAS.md)
...
- **プリセット/重みの rank-IC は `scripts/preset_ic_gate.py` で測る**（ADR-0041・#529）: ADR-0028 の昇格ゲートは #509 と #517 で2回適用されたが**どちらもアドホックなスクリプトで、実体が残らなかった**。標準化経路の選択（消費側の `fit_zscore_stats`＋`normalize_transform` か学習系の `fit_feature_columns` か）で測るものが本番と別物になるため、**書き直さずこの CLI を使う**。パネルは毎晩伸びるので過去の実測と比べるときは `--until <ym>` で期を揃える（揃えないと 0.002 程度ずれて「手続きが違う」ように見える）。**ADR に並ぶ p=0.001 はブートストラップの下限**（`2/(n_boot+1)`）であって強さではない。**重みの推定は `scripts/preset_weight_walkforward.py`**（ADR-0059・#625）＝昇格の根拠は OOF の対比較で、最終重みを本 CLI で測るのは in-sample なので根拠にしない。
- **探索軸は `momentum_gate` で共通域を測ってから足す**（ADR-0050・#592・#604）: `hyperparameter_search` は各候補を**その候補自身の母集団**で評価するため、母集団を動かす軸（`use_momentum`/`momentum_window` 等）はスコアと交絡し、探索は必ず「縮む側」を選ぶ。窓は `--windows`、M-1 のマクロ有無は `--macro`、交互作用の有無は `--interactions`、BIC の列数上限は `--max-features`、TTM 行の昇格ゲートは `--fin-rows`、M-1 の目的変数の月平均除去は `--demean-target`（**6モードは同時指定不可**＝軸を2つ振ると分離できない・#615）。**検証スクリプトの財務は `use_fin_rows` に従う**（`_load_financials` が直に読んでいた間は包んでも黙って通期を測った・#424）。**母集団を動かすかは構造から推論せず測る**——`use_macro` は strict でも実データのマクロに欠損が無く1行も落とさなかった（#604）。**`--smoke` の共通域は間引きで壊れるので読まない**。
