...
- **探索軸は `momentum_gate` で共通域を測ってから足す**（ADR-0050・#592・#604）: `hyperparameter_search` は各候補を**その候補自身の母集団**で評価するため、母集団を動かす軸（`use_momentum`/`momentum_window` 等）はスコアと交絡し、探索は必ず「縮む側」を選ぶ。窓は `--windows`、M-1 のマクロ有無は `--macro`、交互作用の有無は `--interactions`、BIC の列数上限は `--max-features`、TTM 行の昇格ゲートは `--fin-rows`（**5モードは同時指定不可**＝軸を2つ振ると分離できない・#615）。**検証スクリプトの財務は `use_fin_rows` に従う**（`_load_financials` が直に読んでいた間は包んでも黙って通期を測った・#424）。**母集団を動かすかは構造から推論せず測る**——`use_macro` は strict でも実データのマクロに欠損が無く1行も落とさなかった（#604）。**`--smoke` の共通域は間引きで壊れるので読まない**。
