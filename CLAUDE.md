...
- **CF要素名・XBRL ZIP 構造（重要）**: EDINET XBRL type=5 ZIP には**複数の CSV ファイル**が含まれ、CF 合計（`NetCashProvidedByUsedIn*`）は概要ファイルに、CF 明細は別ファイルに存在する。旧実装は最大ファイル1件のみ読んでいたため修正後は ZIP 内の全 CSV を concat して parse する（`fetch_xbrl_csv`）。投資CF の EDINET 標準要素は `NetCashProvidedByUsedIn**Investment**Activities`（Investment）であり旧 `**Investing**Activities` は誤り（修正済み）。
- **capex（設備投資）はラベル照合で取得（重要）**: 設備投資の CF 明細行は**企業独自の拡張要素ID**でタグ付けされることが多く、標準要素ID（`PurchaseOfPropertyPlantAndEquipment`）では捕捉できない（実証: 3000件中0件）。そのため EDINET CSV の**「項目名」列（日本語標準ラベル）**で照合する（`_match_capex_by_label` / `CAPEX_LABEL_*` 定数）。「有形固定資産の取得による支出」「有形固定資産及び無形固定資産の取得による支出」を捕捉し、「売却による収入」「無形固定資産のみ」は除外。capex は支出＝負（`-abs(val)`）で統一。実ラベルの確認は `_pipeline_gh.py --diagnose-cf` で可能。
- **CF NULL 補完の運用**: 既存 NULL データは `refill-cf.yml` で補完する。**自動繰り返しスケジュール**（毎日 UTC 16:00 = JST 01:00）で `cf_net_change_cash IS NULL`（充足率100%の標準要素＝未補完マーカー）を対象に1回 6000件ずつ補完し、補完されると対象から外れて自然終了する（**capex を対象マーカーにすると設備投資が無い企業を永久に再取得するため不可**）。手動起動は `workflow_dispatch` の `mode`（refill / capex-only / diagnose）で制御。`capex-only` は通常補完済みだが capex 未取得のレコードを一度だけ補完する手動ワンショット専用（スケジュールには載せない）。全件補完完了後（ログ artifact の `remaining=0`）はスケジュールを無効化してよい。`company.html` の CF タブは投資CF/設備投資 未収集時に注記を表示する。
